# AUTORUN-2 (soak8-e1acfe6) -- first-admin setup, three channels, program cycling, start playout.
...
$log   = @("# AUTORUN-2 station setup + three-channel soak", "- mission: soak8-e1acfe6", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-2 blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  $log += 'STALLED: no staff token. Ship AUTORUN-4 with a recovery path.'
  $log += ''
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-2 blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  $log += 'STALLED: no sample videos in the kit (expected 4 under samples\).'
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-2 blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
# Program cycling: each channel walks all four videos, one program change every
# 20 minutes, for 8 hours (24 slots per channel), with the three channels offset
# 0 / 7 / 14 minutes so their transitions never coincide.
$startUtc = (Get-Date).ToUniversalTime()
$sched = @("# soak8-e1acfe6 channel schedule", "- host: $env:COMPUTERNAME", "- soak start (UTC): $($startUtc.ToString('o'))",
           "- program change: every 20 minutes per channel, 24 slots (8 h)",
...
$log += "schedule items created: $made (of $(3 * 24) planned)"
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE.md' -Message "test: autorun-2 channel schedule $stamp soak8-e1acfe6"
$startDoc = @("# soak8-e1acfe6 SOAK START", "", "- host: $env:COMPUTERNAME",
  "- soak clock start (UTC): $($startUtc.ToString('o'))",
  "- planned duration: 8 hours (final verdict at T+8h; polling continues forever after)",
...
Save-Report -Lines $startDoc -Name 'SOAK-START.md' -Message "test: autorun-2 soak start $stamp soak8-e1acfe6"
Save-Report -Lines $log -Name "AUTORUN-2-RESULT-$stamp.md" -Message "test: autorun-2 result $stamp soak8-e1acfe6"
exit 0
