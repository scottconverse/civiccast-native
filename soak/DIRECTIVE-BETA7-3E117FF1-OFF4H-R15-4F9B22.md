# Beta.7 four-hour physical soak R15
...
`soak/autorun/AUTORUN-BETA7-3E117FF1-OFF4H-R15-4F9B22.ps1`
...
R15 harness into a new mission root, creates one elevated once-only Scheduled
...
station because it required PowerShell 7 on a Windows PowerShell 5 tester. R15
...
converting a Windows path to a Git path. R15 performs literal string replacement
...
launches are disabled. R15 checks that actual property and its regression fixture
...
commit while the launcher retained its pre-adoption source-anchor object. R15
...
plan into the driver's bounded overlap replacement path. R15 loads the R5 plan
...
disabled. R15 removes that barrier and restores the earlier proven shutdown
...
output on the same live channel. R15 binds that archive by SHA-256 and corrects
...
coordinator while the harness compared raw file bytes. R14 corrected that and
reached healthy controlled transitions on all three channels, but its grader
falsely required independently sliced stdout and stderr histories to contain
the same number of commits. The exact R14 evidence contains four complete
stdout commits and one complete stderr diagnostic commit per channel, with
post-commit output and no worker exit, stall, or 56/74 partial graph. R15 removes
only that invalid cross-file count comparison. It replays those six exact R14
logs and an exact beta.5 Blackwell death log in self-test; missing preroll,
missing handoff, 56/74 topology, clean worker exit, or missing later output
still fail.
...
A passing R15 run authorizes evaluation of the four-hour gate only. It does not
