...
if($DryRun){Write-Output 'DRYRUN: verify installed identity; reuse return repo; replace overlapping test schedule through supported API; serialize strict admission and measured probes; run full ON/OFF R5. No actions performed.';return}
...
$outputRoot=Join-Path $missionRoot 'physical-soak-r5'
$returnRepo=[string]$id.return_repo_path
$relativeRoot='soak/beta7-a963c39cc44e2643065a818aac0206b110d515d4/physical-soak-r5'
...
function Publish-SoakJobReport([switch]$WithArchive){
    ...
    if($WithArchive -and (Test-Path -LiteralPath $outputRoot)){
        ...
        try{
            ...
            $jobLog=Join-Path $missionRoot 'physical-soak-r5-job.log'
...
try{
    ...
    foreach($field in @('candidate_source_sha','build_source_sha','gate_a_source_sha','native_app_payload_source_sha')){if([string]$id.$field -cne $expectedSourceSha){throw "R5 identity field $field is not the exact authorized a963 candidate."}}
    ...
    if(-not(Test-Path -LiteralPath (Join-Path $returnRepo '.git'))){throw 'Existing verified return checkout is absent; R5 does not create a replacement.'}
    Publish-SoakJobReport
    $transcript=Join-Path $missionRoot 'physical-soak-r5-job.log'
    ...
    Write-Beta5JsonAtomic $report (Join-Path $missionRoot 'physical-soak-r5-job.json')
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{Publish-SoakJobReport -WithArchive}
        catch{
            [IO.File]::WriteAllText((Join-Path $missionRoot 'physical-soak-r5-publish-error.txt'),[string]$_.Exception.Message)
            try{Publish-SoakJobReport}catch{[IO.File]::AppendAllText((Join-Path $missionRoot 'physical-soak-r5-publish-error.txt'),"`nStatus publication also failed: "+[string]$_.Exception.Message)}
        }
    }
}
