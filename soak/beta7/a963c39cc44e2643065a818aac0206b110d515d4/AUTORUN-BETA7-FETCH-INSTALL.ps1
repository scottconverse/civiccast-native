...
if ($DryRun) {
    Write-Host "[DRYRUN] verify manifest hash $($id.expected_manifest_sha256), every manifest entry, one signed $($id.expected_version) root installer, then normal upgrade to $installRoot"
    return
}
...
if (-not $product) { throw "Installed Programs identity does not contain the expected CivicCast $($id.expected_version) version." }
