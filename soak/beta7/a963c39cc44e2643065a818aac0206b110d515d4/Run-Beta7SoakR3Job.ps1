...
if($DryRun){Write-Output 'DRYRUN: verify installed identity; reuse return repo; replace overlapping test schedule through supported API; serialize strict admission and measured probes; run full ON/OFF R3. No actions performed.';return}
...
$outputRoot=Join-Path $missionRoot 'physical-soak-r3'
$returnRepo=[string]$id.return_repo_path
$relativeRoot='soak/beta7-a963c39cc44e2643065a818aac0206b110d515d4/physical-soak-r3'
...
function Publish-SoakJobReport([switch]$WithArchive){
    ...
    if($WithArchive -and (Test-Path -LiteralPath $outputRoot)){
        ...
        try{
            ...
            $jobLog=Join-Path $missionRoot 'physical-soak-r3-job.log'
...
try{
    ...
    if(-not(Test-Path -LiteralPath (Join-Path $returnRepo '.git'))){throw 'Existing verified return checkout is absent; R3 does not create a replacement.'}
    Publish-SoakJobReport
    $transcript=Join-Path $missionRoot 'physical-soak-r3-job.log'
    ...
    $r1Plan=@(Get-Content -LiteralPath (Join-Path $r1Runs[0].FullName 'published-plan.json') -Raw | ConvertFrom-Json)
    ...
    Write-Beta5JsonAtomic $report (Join-Path $missionRoot 'physical-soak-r3-job.json')
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{Publish-SoakJobReport -WithArchive}
        catch{
            [IO.File]::WriteAllText((Join-Path $missionRoot 'physical-soak-r3-publish-error.txt'),[string]$_.Exception.Message)
            try{Publish-SoakJobReport}catch{[IO.File]::AppendAllText((Join-Path $missionRoot 'physical-soak-r3-publish-error.txt'),"`nStatus publication also failed: "+[string]$_.Exception.Message)}
        }
    }
}
