...
if($DryRun){Write-Output 'DRYRUN: verify installed identity; reuse return repo; replace overlapping test schedule through supported API; serialize strict admission and measured probes; run full ON/OFF R6. No actions performed.';return}
...
$outputRoot=Join-Path $missionRoot 'physical-soak-r6'
$returnRepo=[string]$id.return_repo_path
$relativeRoot='soak/beta7-a963c39cc44e2643065a818aac0206b110d515d4/physical-soak-r6'
...
function Publish-SoakJobReport([switch]$WithArchive){
    ...
    if($WithArchive -and (Test-Path -LiteralPath $outputRoot)){
        ...
        try{
            ...
            $jobLog=Join-Path $missionRoot 'physical-soak-r6-job.log'
...
try{
    ...
    if(-not(Test-Path -LiteralPath (Join-Path $returnRepo '.git'))){throw 'Existing verified return checkout is absent; R6 does not create a replacement.'}
    Publish-SoakJobReport
    $transcript=Join-Path $missionRoot 'physical-soak-r6-job.log'
    ...
    $r5Runs=@(Get-ChildItem -LiteralPath (Join-Path $missionRoot 'physical-soak-r5') -Directory)
    if($r5Runs.Count -ne 1){throw 'Preserved R5 output does not contain exactly one source run.'}
    $r5Identity=Get-Content -LiteralPath (Join-Path $r5Runs[0].FullName 'IDENTITY.json') -Raw | ConvertFrom-Json
    if([string]$r5Identity.source_sha -cne $expectedSourceSha){throw 'Preserved R5 plan belongs to a different source candidate.'}
    $r5PlanData=Get-Content -LiteralPath (Join-Path $r5Runs[0].FullName 'published-plan.json') -Raw | ConvertFrom-Json
    $r5Plan=@($r5PlanData)
    $allowedScheduleIds=@($r5Plan | ForEach-Object {[string]$_.schedule_id})
    if($r5Plan.Count -ne 180 -or @($allowedScheduleIds | Where-Object {-not $_}).Count -or @($allowedScheduleIds | Sort-Object -Unique).Count -ne 180){throw 'Preserved R5 plan does not contain 180 unique schedule IDs.'}
    if(@($r5Plan.channel_id | Sort-Object -Unique).Count -ne 3 -or @($r5Plan.channel_id | Where-Object {$_ -notin @('public','education','government')}).Count){throw 'Preserved R5 plan channel binding is invalid.'}
    & (Join-Path $PSScriptRoot 'Run-BetaExistingTesterSoakR6.ps1') -BaseUrl 'http://127.0.0.1:8000' -OutputRoot $outputRoot -SourceSha $id.candidate_source_sha -Minutes 120 -Mode Both -ReplaceOverlappingTestSchedule -AllowedOverlapScheduleIds $allowedScheduleIds -RequireTransportAdmission -ExpectedElementsPerPhase $expectedElements -OnPhaseStarted $phaseReceipt *> $transcript
    ...
    Write-Beta5JsonAtomic $report (Join-Path $missionRoot 'physical-soak-r6-job.json')
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{Publish-SoakJobReport -WithArchive}
        catch{
            [IO.File]::WriteAllText((Join-Path $missionRoot 'physical-soak-r6-publish-error.txt'),[string]$_.Exception.Message)
            try{Publish-SoakJobReport}catch{[IO.File]::AppendAllText((Join-Path $missionRoot 'physical-soak-r6-publish-error.txt'),"`nStatus publication also failed: "+[string]$_.Exception.Message)}
        }
    }
}
