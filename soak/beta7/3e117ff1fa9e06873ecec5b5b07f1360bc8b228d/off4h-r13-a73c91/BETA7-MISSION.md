...
Status: **R13 ONCE-ONLY PHYSICAL MISSION; BETA.7 REMAINS UNPUBLISHED**. R5 is
...
PowerShell 7 (`pwsh`), which is not installed on `DESKTOP-VBMA6O5`. R13 keeps
...
coordinator-only dual-runtime mode. R13 may run exactly once from its
...
a Git path. R13 uses literal string replacement and executes that exact source
...
commit, and subsequent output for the 33-element programme graph. R13 records
...
- nonce: `BETA7-3E117FF1-OFF4H-R13-A73C91`
...
back and must exactly match the published five-minute R13-owned plan. Final
