...
function Get-TSDuckR13RequiredInteger {
...
function Get-TSDuckR13ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR13RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR13RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR13RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR13RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR13MetricsVerdict {
