...
function Get-TSDuckR11RequiredInteger {
...
function Get-TSDuckR11ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR11RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR11RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR11RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR11RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR11MetricsVerdict {
