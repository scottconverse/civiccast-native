...
function Get-R14OwnedActiveScheduleRows($Rows, [string]$OwnedNotes) {
...
function Remove-RemainingOwnedSchedule {
    ...
    foreach($channel in $channels) {
        ...
        $discovered += @(Get-R14OwnedActiveScheduleRows $rows $ownedNotes | ForEach-Object {
            if(-not $_.id){throw "R14-owned active schedule row on $($channel.id) has no ID."}
    ...
    if($duplicateIds.Count){throw "R14-owned active schedule IDs are duplicated across channel queries: $($duplicateIds.Name -join ',')."}
    ...
    foreach($channel in $channels){
        $rows=@(Invoke-Station GET "/api/staff/schedule?channel_id=$($channel.id)")
        $remaining += @(Get-R14OwnedActiveScheduleRows $rows $ownedNotes)
    }
    if($remaining.Count){throw "$($remaining.Count) active R14-owned schedule rows remain after cleanup."}
...
'@
    ...
    $cleanupSelected=@(Get-R14OwnedActiveScheduleRows $cleanupFixtures $ownedNotes)
...
. (Join-Path $PSScriptRoot 'Beta7TransportProbeR14.ps1')
...
function Invoke-PremeasurementTopologyProof([string]$Phase, $Plan, $Offsets, $ExpectedElements) {
    ...
    foreach($channel in $channels){
        ...
        $programCommit=$stderrText.LastIndexOf('stage=committed elements=33',[StringComparison]::Ordinal)
        if($programCommit -lt 0 -or $stderrText.IndexOf('CTRL output:',$programCommit,[StringComparison]::Ordinal) -lt 0){throw "$Phase single-programme commit for $($channel.id) has no subsequent output proof."}
...
try {
    ...
    foreach ($phase in $phases) {
        ...
        # R14 transport/topology evidence is deliberately captions OFF; caption
