...
function Get-TSDuckR14RequiredInteger {
...
function Get-TSDuckR14ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR14RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR14RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR14RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR14RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR14MetricsVerdict {
