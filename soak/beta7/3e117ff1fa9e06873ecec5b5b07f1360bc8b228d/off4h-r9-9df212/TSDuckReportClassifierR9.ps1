...
function Get-TSDuckR9RequiredInteger {
...
function Get-TSDuckR9ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR9RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR9RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR9RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR9RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR9MetricsVerdict {
