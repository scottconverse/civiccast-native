...
Status: **R9 ONCE-ONLY PHYSICAL MISSION; BETA.7 REMAINS UNPUBLISHED**. R5 is
...
PowerShell 7 (`pwsh`), which is not installed on `DESKTOP-VBMA6O5`. R9 keeps
...
coordinator-only dual-runtime mode. R9 may run exactly once from its
...
a Git path. R9 uses literal string replacement and executes that exact source
...
- nonce: `BETA7-3E117FF1-OFF4H-R9-9DF212`
...
back and must exactly match the published five-minute R9-owned plan. Final
