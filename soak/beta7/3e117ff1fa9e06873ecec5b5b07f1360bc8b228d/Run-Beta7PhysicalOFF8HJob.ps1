...
if($DryRun){$null=& (Join-Path $PSScriptRoot 'Assert-Beta7CandidateBinding.ps1') -IdentityPath $IdentityPath;Write-Output 'DRYRUN VERIFIED: exact finalized identity and 480-minute captions-OFF job plan passed; no mutation performed.';return}
...
$outputRoot=Join-Path $missionRoot 'physical-soak-off8h-r3-a91e4c'
$returnRepo=[string]$id.return_repo_path
$relativeRoot='soak/beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d/physical-soak-off8h-r3-a91e4c'
if(Test-Path -LiteralPath $outputRoot){throw 'Physical soak output already exists. Preserve it; do not replay this job.'}
$report=[ordered]@{hostname=[string]$env:COMPUTERNAME;source_sha=[string]$id.candidate_source_sha;mission_nonce=[string]$id.mission_nonce;job_started_utc=[datetime]::UtcNow.ToString('o');job_state='STARTED';minutes=480;phases=@('OFF');result=$null;error=$null}
function Publish-SoakJobReport([switch]$WithArchive){
    ...
    if($WithArchive -and (Test-Path -LiteralPath $outputRoot)){
        ...
        try{
            ...
            $jobLog=Join-Path $missionRoot 'physical-soak-off8h-r3-a91e4c-job.log'
...
try{
    ...
    if([int]$id.planned_seconds -ne 28800){throw 'Identity is not bound to the required eight measured captions-OFF hours.'}
    ...
    $transcript=Join-Path $missionRoot 'physical-soak-off8h-r3-a91e4c-job.log'
    ...
    & (Join-Path $PSScriptRoot 'Run-BetaExistingTesterSoakR6.ps1') -BaseUrl 'http://127.0.0.1:8000' -OutputRoot $outputRoot -SourceSha $id.candidate_source_sha -Minutes 480 -Mode OFF -ReplaceOverlappingTestSchedule -RequireTransportAdmission -ExpectedElementsPerPhase $expectedElements -OnPhaseStarted $phaseReceipt *> $transcript
    ...
    Write-Beta5JsonAtomic $report (Join-Path $missionRoot 'physical-soak-off8h-r3-a91e4c-job.json')
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{Publish-SoakJobReport -WithArchive}
        catch{
            [IO.File]::WriteAllText((Join-Path $missionRoot 'physical-soak-off8h-r3-a91e4c-publish-error.txt'),[string]$_.Exception.Message)
            try{Publish-SoakJobReport}catch{[IO.File]::AppendAllText((Join-Path $missionRoot 'physical-soak-off8h-r3-a91e4c-publish-error.txt'),"`nStatus publication also failed: "+[string]$_.Exception.Message)}
        }
    }
}
