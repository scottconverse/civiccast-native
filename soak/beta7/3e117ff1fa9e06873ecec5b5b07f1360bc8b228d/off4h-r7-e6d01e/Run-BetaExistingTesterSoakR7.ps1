...
function Get-R7OwnedActiveScheduleRows($Rows, [string]$OwnedNotes) {
...
function Remove-RemainingOwnedSchedule {
    ...
    foreach($channel in $channels) {
        ...
        $discovered += @(Get-R7OwnedActiveScheduleRows $rows $ownedNotes | ForEach-Object {
            if(-not $_.id){throw "R7-owned active schedule row on $($channel.id) has no ID."}
    ...
    if($duplicateIds.Count){throw "R7-owned active schedule IDs are duplicated across channel queries: $($duplicateIds.Name -join ',')."}
    ...
    foreach($channel in $channels){
        $rows=@(Invoke-Station GET "/api/staff/schedule?channel_id=$($channel.id)")
        $remaining += @(Get-R7OwnedActiveScheduleRows $rows $ownedNotes)
    }
    if($remaining.Count){throw "$($remaining.Count) active R7-owned schedule rows remain after cleanup."}
...
'@
    ...
    $cleanupSelected=@(Get-R7OwnedActiveScheduleRows $cleanupFixtures $ownedNotes)
...
. (Join-Path $PSScriptRoot 'Beta7TransportProbeR7.ps1')
...
try {
    ...
    foreach ($phase in $phases) {
        ...
        # R7 transport/topology evidence is deliberately captions OFF; caption
