...
function Get-TSDuckR7RequiredInteger {
...
function Get-TSDuckR7ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR7RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR7RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR7RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR7RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR7MetricsVerdict {
