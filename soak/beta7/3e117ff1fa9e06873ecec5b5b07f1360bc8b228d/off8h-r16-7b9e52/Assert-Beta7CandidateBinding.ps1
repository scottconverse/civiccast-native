...
if([string]$id.mission_nonce -cne 'BETA7-3E117FF1-OFF8H-R16-7B9E52'){throw 'Mission nonce mismatch.'}
if([string]$id.task_name -cne 'CivicCast-Beta7-3e117ff1-OFF8H-R16-7B9E52'){throw 'Scheduled-task identity mismatch.'}
if([string]$id.evidence_relative_root -cne 'soak/beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d/physical-soak-off8h-r16-7b9e52'){throw 'Evidence-root identity mismatch.'}
...
if($topology.Count -ne 3 -or @($topology|Where-Object{(@($_.Value|ForEach-Object{[int]$_}|Sort-Object -Unique)-join ',') -cne '33,132'}).Count){throw 'Physical topology must bind captions-OFF elements=33 and elements=132 on all three channels.'}
if("$($id.topology_provenance.kind)" -cne 'current-candidate-physical-shape-bound' -or "$($id.topology_provenance.evidence_path)" -cne 'soak/beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d/physical-soak-off4h-r12-db2617/evidence.zip' -or "$($id.topology_provenance.evidence_sha256)" -cne '6bed7d091c1c82bd0fcfd3fe2d99edab4bda66c220d03c96d5f1290a67239f01' -or -not[bool]$id.topology_provenance.current_candidate_must_reestablish_before_measurement){throw 'Topology provenance is not bound to the preserved exact-candidate R12 evidence and current-run reestablishment.'}
if([int]$id.topology_provenance.healthy_elements_by_shape.single_programme -ne 33 -or [int]$id.topology_provenance.healthy_elements_by_shape.twelve_segment_slate -ne 132){throw 'Healthy captions-OFF topology shapes must be exactly 33 and 132.'}
$failed=@($id.topology_provenance.rejected_historical_partial_elements|ForEach-Object{[int]$_}|Sort-Object -Unique)
if(($failed -join ',') -cne '56,74'){throw 'Known beta.5 partial topology counts must be exactly 56 and 74.'}
