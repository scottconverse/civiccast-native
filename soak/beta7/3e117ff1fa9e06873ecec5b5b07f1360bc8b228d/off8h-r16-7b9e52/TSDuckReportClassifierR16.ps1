...
function Get-TSDuckR16RequiredInteger {
...
function Get-TSDuckR16ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR16RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR16RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR16RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR16RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR16MetricsVerdict {
