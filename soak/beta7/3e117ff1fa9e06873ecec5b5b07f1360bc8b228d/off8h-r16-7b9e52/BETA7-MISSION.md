...
Status: **R16 ONCE-ONLY PHYSICAL MISSION; BETA.7 REMAINS UNPUBLISHED**. R5 is
...
PowerShell 7 (`pwsh`), which is not installed on `DESKTOP-VBMA6O5`. R16 keeps
...
coordinator-only dual-runtime mode. R16 is once-only and may run exactly once from its
...
a Git path. R16 uses literal string replacement and executes that exact source
...
commit, and subsequent output for the 33-element programme graph. R16 records
...
tester and CRLF on the coordinator, while R13 compared raw manifest bytes. R16
...
R14 reached healthy controlled transitions on all three channels, then falsely
failed because stdout and stderr were sliced from independent byte offsets and
the grader required their commit counts to match. The exact R14 slices contain
four complete stdout transactions and one complete stderr diagnostic transaction
per channel, followed by output. R16 removes only that invalid comparison and
replays the exact R14 evidence plus an exact beta.5 Blackwell death log in its
self-test. Every transaction, topology, worker-exit, stall, source, PID, and
post-commit-output requirement remains enforced.

R16 replaces byte-offset-only capture with rotation-aware checkpoints. Each active
log checkpoint records its length and SHA-256 prefix. At collection the harness
finds exactly one matching generation, captures its suffix, then appends every
newer retained generation in chronological order. CivicCast retains ten backups at
10 MiB; a missing, ambiguous, truncated, or evicted checkpoint fails the mission
closed, preserving the F1 worker/transaction, F2 timing, and F3 output rules.
...
- nonce: `BETA7-3E117FF1-OFF8H-R16-7B9E52`
...
and runs exactly 480 measured minutes captions OFF.
...
on all three R5 channels. It does not call the unavailable `skip` plugin. Admission and sixteen
serialized measured rounds per channel use explicit feasible offsets from 0
through 450 minutes. Measured sync, transport, and continuity errors remain
zero tolerance.
...
back and must exactly match the published five-minute R16-owned plan. Final
