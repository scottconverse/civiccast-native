...
if($DryRun){Write-Output 'DRYRUN VERIFIED: exact R16 identity, harness, and authoritative completion plan passed; no mutation performed.';return}
if($env:COMPUTERNAME -ine 'DESKTOP-VBMA6O5'){throw 'This R16 soak job targets DESKTOP-VBMA6O5 only.'}
...
$report=[ordered]@{schema='civiccast-beta7-physical-job-v3';hostname=[string]$env:COMPUTERNAME;source_sha=[string]$id.candidate_source_sha;mission_nonce=[string]$id.mission_nonce;harness_source_commit=[string]$id.harness_source_commit;harness_manifest_sha256=$packageProof.manifest_sha256;evidence_relative_path=$archiveRelative;completion_relative_path=$completionRelative;job_started_utc=[datetime]::UtcNow.ToString('o');job_state='STARTED';minutes=480;phases=@('OFF');result=$null;error=$null}
...
function New-RunLatch {
    ...
    }catch{throw 'R16 nonce has already been consumed or its durable latch could not be created.'}
...
function Disable-OwnTask {
    ...
    if([bool]$observed.Settings.Enabled){throw 'R16 task remained enabled for future launches at entry.'}
    $report.task_entry=@{previous_state=[string]$task.State;observed_state=[string]$observed.State;enabled_after_disable=[bool]$observed.Settings.Enabled;at=[datetime]::UtcNow.ToString('o')}
}
...
try{
    ...
    if("$($id.gate_a_verdict)" -ne 'PASS' -or [int]$id.planned_seconds -ne 28800){throw 'Gate A or eight-hour duration binding is invalid.'}
    ...
    foreach($entry in $id.actual_runtime_file_sha256.PSObject.Properties){
        ...
        if((Get-FileHash -LiteralPath $filePath -Algorithm SHA256).Hash.ToLowerInvariant() -ne "$($entry.Value)"){throw "Installed runtime changed after R16 adoption: $($entry.Name)"}
    }
    ...
    if(Test-Path -LiteralPath $outputRoot){throw 'R16 output already exists; preserve it and do not replay.'}
    if(Test-Path -LiteralPath $returnRepo){throw 'R16 return checkout already exists; preserve it and do not replay.'}
    ...
    if($cloneExit -ne 0){throw "Could not clone the R16 evidence return branch: $($cloneOutput -join ' ')"}
    ...
    Write-Job
    $report.started_commit=Sync-And-Push @($jobRelative) 'test: beta.7 R16 physical soak STARTED'
    ...
    Write-Job
    $report.phase_commit=Sync-And-Push @($jobRelative) 'test: beta.7 R16 measured phase READY'
    ...
    $r5InvalidationPath='C:\CivicCastSoak\repo\soak\beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d\physical-soak-off4h-r5-e27a91\INVALIDATED.json'
    $r5Invalidation=Get-Content -LiteralPath $r5InvalidationPath -Raw|ConvertFrom-Json
    $r5PlanPath=[string]$r5Invalidation.verification.local_r5.published_plan.path
    if(-not(Test-Path -LiteralPath $r5PlanPath -PathType Leaf)){throw 'Preserved R5 plan named by the invalidation receipt is absent.'}
    $r5PlanHash=(Get-FileHash -LiteralPath $r5PlanPath -Algorithm SHA256).Hash.ToLowerInvariant()
    if($r5PlanHash -cne [string]$r5Invalidation.verification.local_r5.published_plan.sha256){throw 'Preserved R5 plan bytes do not match the invalidation receipt.'}
    $r5PlanData=Get-Content -LiteralPath $r5PlanPath -Raw|ConvertFrom-Json
    $r5Plan=@($r5PlanData|ForEach-Object{$_})
    $allowedScheduleIds=@($r5Plan|ForEach-Object{[string]$_.schedule_id})
    if($r5Plan.Count -ne 180 -or @($allowedScheduleIds|Where-Object{$_}|Sort-Object -Unique).Count -ne 180){throw 'Preserved R5 plan does not contain exactly 180 unique schedule IDs.'}
    & (Join-Path $PSScriptRoot 'Run-BetaExistingTesterSoakR16.ps1') -BaseUrl 'http://127.0.0.1:8000' -OutputRoot $outputRoot -SourceSha $id.candidate_source_sha -Minutes 480 -Mode OFF -ReplaceOverlappingTestSchedule -AllowedOverlapScheduleIds $allowedScheduleIds -RequireTransportAdmission -ExpectedElementsPerPhase $expectedElements -OnPhaseStarted $phaseReceipt *> $jobLog
    $runs=@(Get-ChildItem -LiteralPath $outputRoot -Directory)
    if($runs.Count -ne 1){throw 'R16 physical output does not contain exactly one run.'}
    $run=$runs[0].FullName
    $report.result=Get-Content -LiteralPath (Join-Path $run 'VERDICT.json') -Raw|ConvertFrom-Json
    if("$($report.result.verdict)" -ne 'TIMING_LIFETIME_TRANSPORT_PASS'){throw 'R16 physical checks did not pass.'}
    if(-not(Test-Path -LiteralPath (Join-Path $run 'STOP-VERIFIED.json') -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $run 'STOP-FAILED.json') -PathType Leaf) -or -not(Test-Path -LiteralPath (Join-Path $run 'SCHEDULE-CLEANUP-VERIFIED.json') -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $run 'SCHEDULE-CLEANUP-FAILED.json') -PathType Leaf)){throw 'R16 terminal channel and owned-schedule cleanup is not verified.'}
    ...
    Write-Job;$report.evidence_commit=Sync-And-Push @($jobRelative,$archiveRelative) 'test: beta.7 R16 collection and evidence uploaded';$evidencePushed=$true
    ...
    $report.completion_commit=Sync-And-Push @($jobRelative,$completionRelative) 'test: beta.7 R16 authoritative PASS completion'
    ...
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{
            ...
            $report.failure_commit=Sync-And-Push $failurePaths 'test: beta.7 R16 physical soak FAIL'
...
if($report.job_state -ne 'PASS'){throw "R16 physical soak did not pass: $finalFailure"}
