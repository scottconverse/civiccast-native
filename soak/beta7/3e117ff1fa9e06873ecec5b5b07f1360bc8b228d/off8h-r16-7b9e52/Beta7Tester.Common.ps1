...
function Get-Beta5Identity {
    ...
    $required = @(
        ...
        'state_sample_interval_seconds', 'caption_status_interval_seconds',
        'hardware_sample_interval_seconds', 'transport_round_interval_seconds',
        'transport_round_offsets_seconds', 'channels', 'expected_elements_per_phase',
        'topology_provenance', 'expected_harness_manifest_sha256', 'harness_source_commit',
        'r5_invalidation_receipt_sha256', 'r5_invalidation_remote_commit'
    )
    ...
    if ([int] $id.planned_seconds -ne 28800 -or [int]$id.minutes_per_phase -ne 480) { throw 'R16 duration is not exactly one eight-hour captions-OFF phase.' }
    if ([int] $id.state_sample_interval_seconds -ne 10 -or [int] $id.caption_status_interval_seconds -ne 30 -or [int] $id.hardware_sample_interval_seconds -ne 30 -or [int] $id.transport_round_interval_seconds -ne 1800) { throw 'R16 sampling intervals are not the reviewed 10/30/30/1800-second contract.' }
    $roundOffsets=@($id.transport_round_offsets_seconds | ForEach-Object {[int]$_})
    if($roundOffsets.Count -ne 16 -or ($roundOffsets -join ',') -cne '0,1800,3600,5400,7200,9000,10800,12600,14400,16200,18000,19800,21600,23400,25200,27000'){throw 'R16 transport offsets are not the reviewed sixteen-round eight-hour cadence.'}
if("$($id.expected_harness_manifest_sha256)" -notmatch '^[0-9a-f]{64}$' -or "$($id.expected_harness_manifest_sha256)" -ceq ('0' * 64)){throw 'Expected harness manifest SHA-256 is absent, zero, or invalid.'}
if("$($id.harness_source_commit)" -notmatch '^[0-9a-f]{40}$' -or "$($id.harness_source_commit)" -ceq ('0' * 40)){throw 'Harness source commit is absent, zero, or invalid.'}
if("$($id.r5_invalidation_receipt_sha256)" -notmatch '^[0-9a-f]{64}$' -or "$($id.r5_invalidation_receipt_sha256)" -ceq ('0' * 64) -or "$($id.r5_invalidation_remote_commit)" -notmatch '^[0-9a-f]{40}$' -or "$($id.r5_invalidation_remote_commit)" -ceq ('0' * 40)){throw 'R5 invalidation provenance is absent, zero, or invalid.'}
    ...
    if ($expectedPhases.Count -ne 1 -or [string]$expectedPhases[0].Name -cne 'OFF') { throw 'Identity expected_elements_per_phase must contain exactly the captions-OFF phase.' }
    foreach ($phase in $expectedPhases) {
        ...
        foreach ($topology in $expectedTopology) {
            ...
            $normalized=@($counts | ForEach-Object {[int]$_} | Sort-Object -Unique)
            if(($normalized -join ',') -cne '33,132'){throw "Identity topology expectation for $($phase.Name)/$($topology.Name) must be exactly the reviewed captions-OFF one-programme and twelve-segment shapes 33,132."}
        }
    }
    $failedCounts=@($id.topology_provenance.rejected_historical_partial_elements|ForEach-Object{[int]$_}|Sort-Object -Unique)
    if(($failedCounts -join ',') -cne '56,74'){throw 'Topology provenance must bind the known beta.5 partial counts 56 and 74.'}
