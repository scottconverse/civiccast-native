...
. (Join-Path $PSScriptRoot 'TSDuckReportClassifierR16.ps1')
function Get-Beta7TransportGrade {
    ...
    try {
        ...
        $metrics = Get-TSDuckR16ReportMetrics $Json
...
function Start-Beta7TransportProbe {
    ...
    # The channel is stabilized before this function is called.  Analyze one
    # real 30-second interval; synthetic acquisition discard made the old
    # 35-second probe impossible to serialize on the 30-minute cadence.
    $args += @('-P','until','--seconds','30')
    ...
    return [pscustomobject]@{Process=$process;StartedUtc=$started;DeadlineUtc=$started.AddSeconds(60);Port=$Port;Label=$Label;AnalyzedSeconds=30;ReportPath=$report;StdoutPath=$stdout;StderrPath=$stderr}
}
...
function Add-Beta7TransportLifecycleEvidence {
    ...
    $Result | Add-Member -NotePropertyName analyzed_seconds -NotePropertyValue ([int]$Probe.AnalyzedSeconds) -Force
    return $Result
}
