...
function Set-OwnedChannelsQuiescentIntent {
    foreach ($channel in $owned) {
        ...
        if (-not $config) { throw "Owned channel $channel has no readable config." }
        if (-not [bool]$config.enabled -and [bool]$config.auto_start) { throw "Owned channel $channel has unsafe disabled-plus-auto-start intent." }
        $originalConfig=$config | ConvertTo-Json -Depth 20 -Compress | ConvertFrom-Json
        # Recover a channel left disabled by R11 while keeping automatic start
        # suppressed. Every other stored config value must remain exact.
        $config.enabled=$true; $config.auto_start=$false
        $written = Invoke-Station PUT "/api/staff/egress/channels/$channel/config" $config
        if (-not (Test-QuiescentChannelConfig $written) -or -not (Test-ConfigPreservedForBarrier $originalConfig $written)) { throw "Owned channel $channel quiescent intent did not preserve full config." }
        $readback = Invoke-Station GET "/api/staff/egress/channels/$channel/config"
        if (-not (Test-QuiescentChannelConfig $readback) -or -not (Test-ConfigPreservedForBarrier $originalConfig $readback)) { throw "Owned channel $channel quiescent readback failed." }
        $barrierConfigs[$channel]=$readback | ConvertTo-Json -Depth 20 -Compress | ConvertFrom-Json
    }
}
function Stop-OwnedChannels {
    ...
    do {
        ...
        if ($remaining.Count) { $quietSince=$null }
        elseif ($null -eq $quietSince) { $quietSince=[datetime]::UtcNow }
        ...
        foreach($channel in $remaining) {
            $response=Invoke-Station POST "/api/staff/egress/channels/$channel/commands" @{action='stop'}
            $command=if($response.command){$response.command}else{$response}
            if (-not $command -or "$($command.action)" -ne 'stop' -or -not $command.issued_at) { throw "Repeated stop command for $channel had no verifiable issue receipt." }
            $issued[$channel]=[datetimeoffset]::Parse([string]$command.issued_at).UtcDateTime
        }
...
function Test-Beta7PostCommitOutput([string]$StderrText, [int]$Elements=33) {
    $marker="stage=committed elements=$Elements"
    $commit=$StderrText.LastIndexOf($marker,[StringComparison]::Ordinal)
    return ($commit -ge 0 -and $StderrText.IndexOf('CTRL output:',$commit,[StringComparison]::Ordinal) -ge 0)
}
...
function Get-R16OwnedActiveScheduleRows($Rows, [string]$OwnedNotes) {
...
function Remove-RemainingOwnedSchedule {
    ...
    foreach($channel in $channels) {
        ...
        $discovered += @(Get-R16OwnedActiveScheduleRows $rows $ownedNotes | ForEach-Object {
            if(-not $_.id){throw "R16-owned active schedule row on $($channel.id) has no ID."}
    ...
    if($duplicateIds.Count){throw "R16-owned active schedule IDs are duplicated across channel queries: $($duplicateIds.Name -join ',')."}
    ...
    foreach($channel in $channels){
        $rows=@(Invoke-Station GET "/api/staff/schedule?channel_id=$($channel.id)")
        $remaining += @(Get-R16OwnedActiveScheduleRows $rows $ownedNotes)
    }
    if($remaining.Count){throw "$($remaining.Count) active R16-owned schedule rows remain after cleanup."}
...
. (Join-Path $PSScriptRoot 'Beta7RotationAwareLogsR16.ps1')
if ($SelfTest) {
    $quiescenceFixtures = @(
    ...
    $quiescentConfigFixtures = @(
        @{name='enabled with auto-start suppressed';config=[pscustomobject]@{enabled=$true;auto_start=$false};expected=$true},
        @{name='enabled auto-start channel';config=[pscustomobject]@{enabled=$true;auto_start=$true};expected=$false},
        @{name='disabled channel';config=[pscustomobject]@{enabled=$false;auto_start=$false};expected=$false},
        @{name='missing config';config=$null;expected=$false}
    )
    foreach ($fixture in $quiescentConfigFixtures) {
        if ((Test-QuiescentChannelConfig $fixture.config) -ne $fixture.expected) {
    ...
    $r11DisabledConfig=[pscustomobject]@{sinks=@([pscustomobject]@{latency_ms=2000;uri='udp://127.0.0.1:9001';kind='udp-ts'});fill_policy='slate';auto_start=$false;enabled=$false;channel_id='public'}
    if (-not (Test-ConfigPreservedForBarrier $r11DisabledConfig $onlyAutoStartChanged)) { throw 'R11 disabled-channel recovery comparison rejected an enabled-only repair.' }
    ...
    $freshGrade = Get-Beta7FailureGrades $freshLogs @{public=@(33,132)}
    ...
    $postOffsetGrade = Get-Beta7FailureGrades $postOffsetLogs @{public=@(33,132)}
...
CTRL reload committed (elements=33)
'@}
...
CTRL reload diagnostic: stage=committed elements=33
CTRL output: buffers=300 bytes=56400
'@}
    $goodGrade=Get-Beta7FailureGrades @($goodStdout,$goodStderr) @{public=@(33,132)}
    if(-not $goodGrade.f1_pass){throw "Complete reload transaction fixture failed: $($goodGrade.f1_commit_without_preroll -join '; ')"}
    if(-not (Test-Beta7PostCommitOutput $goodStderr.text 33)){throw 'Complete reload transaction fixture has no post-commit output.'}

    # Replay the exact R14 premeasurement slices that exposed independently
    # truncated stdout/stderr histories. Every stdout transaction must still be
    # complete; every diagnostic transaction present must still prove selector
    # handoff and old-tail detach. Their counts are not comparable because byte
    # offsets are captured independently for each file.
    $r14FixtureLogs=@()
    foreach($fixtureChannel in @('public','education','government')){
        foreach($stream in @('stdout','stderr')){
            $fixturePath=Join-Path $PSScriptRoot "R14-$fixtureChannel-gst-worker.$stream.log"
            if(-not (Test-Path -LiteralPath $fixturePath -PathType Leaf)){throw "R14 evidence fixture is absent: $fixturePath"}
            $r14FixtureLogs += [pscustomobject]@{path=$fixturePath;text=[IO.File]::ReadAllText($fixturePath)}
        }
    }
    $r14Expected=@{public=@(33,132);education=@(33,132);government=@(33,132)}
    $r14Grade=Get-Beta7FailureGrades $r14FixtureLogs $r14Expected
    if(-not $r14Grade.f1_pass -or -not $r14Grade.f3_pass){throw "Exact healthy R14 evidence fixture failed: $($r14Grade.f1_commit_without_preroll -join '; ')"}
    foreach($fixtureChannel in @('public','education','government')){
        $stdoutText=[string](@($r14FixtureLogs|Where-Object {$_.path -match "(?i)R14-$fixtureChannel-gst-worker\.stdout"})[0].text)
        $stderrText=[string](@($r14FixtureLogs|Where-Object {$_.path -match "(?i)R14-$fixtureChannel-gst-worker\.stderr"})[0].text)
        if([regex]::Matches($stdoutText,'CTRL reload committed').Count -ne 4 -or [regex]::Matches($stderrText,'stage=committed').Count -ne 1){throw "R14 evidence fixture no longer reproduces the 4-to-1 slice boundary for $fixtureChannel."}
        if(-not (Test-Beta7PostCommitOutput $stderrText 33)){throw "R14 evidence fixture has no post-commit output for $fixtureChannel."}
    }
    $publicStdout=@($r14FixtureLogs|Where-Object {$_.path -match '(?i)R14-public-gst-worker\.stdout'})[0]
    $publicStderr=@($r14FixtureLogs|Where-Object {$_.path -match '(?i)R14-public-gst-worker\.stderr'})[0]
    $missingPreroll=[pscustomobject]@{path=$publicStdout.path;text=($publicStdout.text -replace '(?m)^CTRL reload: new leg preroll verified \(reload_id=4\).*(?:\r?\n|$)','')}
    if((Get-Beta7FailureGrades @($missingPreroll,$publicStderr) @{public=@(33,132)}).f1_pass){throw 'Exact R14 evidence without the final preroll receipt incorrectly passed.'}
    $missingHandoff=[pscustomobject]@{path=$publicStderr.path;text=($publicStderr.text -replace '(?m)^CTRL reload diagnostic: stage=selector-handoff-confirmed(?:\r?\n|$)','')}
    if((Get-Beta7FailureGrades @($publicStdout,$missingHandoff) @{public=@(33,132)}).f1_pass){throw 'Exact R14 evidence without selector handoff incorrectly passed.'}
    $partialStdout=[pscustomobject]@{path=$publicStdout.path;text=$publicStdout.text.Replace('CTRL reload committed (elements=33)','CTRL reload committed (elements=56)')}
    if((Get-Beta7FailureGrades @($partialStdout,$publicStderr) @{public=@(33,132)}).f1_pass){throw 'Exact R14 evidence with a 56-element partial graph incorrectly passed.'}
    $workerExit=[pscustomobject]@{path=$publicStdout.path;text=$publicStdout.text+"`nWORKER_RESULT {`"error`": null, `"teardown_clean`": true}"}
    if((Get-Beta7FailureGrades @($workerExit,$publicStderr) @{public=@(33,132)}).f1_pass){throw 'Exact R14 evidence with a clean worker exit incorrectly passed.'}
    $commitEnd=$publicStderr.text.LastIndexOf('stage=committed elements=33',[StringComparison]::Ordinal)+'stage=committed elements=33'.Length
    $noLaterOutput=$publicStderr.text.Substring(0,$commitEnd)
    if(Test-Beta7PostCommitOutput $noLaterOutput 33){throw 'Exact R14 evidence without later output incorrectly passed.'}

    # Replay one exact beta.5 Blackwell death log. This is the original release
    # blocker: partial 56/74-element pipelines followed by clean worker exits.
    $beta5Path=Join-Path $PSScriptRoot 'BETA5-blackwell-public-gst-worker.stdout.log'
    if(-not (Test-Path -LiteralPath $beta5Path -PathType Leaf)){throw "Beta.5 Blackwell evidence fixture is absent: $beta5Path"}
    $beta5Log=[pscustomobject]@{path=$beta5Path;text=[IO.File]::ReadAllText($beta5Path)}
    $beta5Grade=Get-Beta7FailureGrades @($beta5Log) @{public=@(33,132)}
    if($beta5Grade.f1_pass -or @($beta5Grade.f1_worker_or_timeout_lines).Count -lt 1){throw 'Exact beta.5 Blackwell clean-worker-exit evidence incorrectly passed.'}
    if(@($beta5Grade.f1_commit_without_preroll|Where-Object {$_ -match 'elements=(?:56|74)|Untrusted or undersized topology'}).Count -lt 1){throw 'Exact beta.5 Blackwell partial topology evidence was not rejected.'}
    $workerErrorGrade=Get-Beta7FailureGrades @([pscustomobject]@{path=$goodStdout.path;text='WORKER_RESULT {"error": null, "teardown_clean": true}'}) @{public=@(33,132)}
    if($workerErrorGrade.f1_pass){throw 'Any WORKER_RESULT fixture incorrectly passed.'}
    $danglingHold=Get-Beta7FailureGrades @([pscustomobject]@{path=$goodStdout.path;text='CTRL reload: new leg stream held at its first buffer (1 stream(s) still to preroll) (reload_id=8)'}) @{public=@(33,132)}
...
'@
    $danglingFire=Get-Beta7FailureGrades @([pscustomobject]@{path=$goodStdout.path;text=$danglingFireText}) @{public=@(33,132)}
    ...
    $danglingDiagnostic=Get-Beta7FailureGrades @([pscustomobject]@{path=$goodStderr.path;text='CTRL reload diagnostic: stage=selector-handoff-confirmed'}) @{public=@(33,132)}
    ...
    $roundSchedule=@(Get-TransportRoundSchedule ([datetime]'2026-09-11T12:00:00Z') 480)
    if($roundSchedule.Count -ne 16 -or (@($roundSchedule | ForEach-Object {$_.offset_minutes}) -join ',') -cne '0,30,60,90,120,150,180,210,240,270,300,330,360,390,420,450'){throw 'Transport cadence fixture did not produce sixteen feasible 30-minute offsets.'}
    ...
    $cleanupSelected=@(Get-R16OwnedActiveScheduleRows $cleanupFixtures $ownedNotes)
...
'@};stderr=$goodStderr},
        ...
        @{name='missing old-tail detach';stdout=$goodStdout;stderr=[pscustomobject]@{path=$goodStderr.path;text=($goodStderr.text -replace '(?m)^CTRL reload diagnostic: stage=old-tail-detached\r?\n?','')}}
    )) {
        $badGrade=Get-Beta7FailureGrades @($bad.stdout,$bad.stderr) @{public=@(33,132)}
        if($badGrade.f1_pass){throw "Invalid reload transaction fixture passed: $($bad.name)"}
    }
    $slateStdout=[pscustomobject]@{path=$goodStdout.path;text=$goodStdout.text.Replace('elements=33','elements=132')}
    $slateStderr=[pscustomobject]@{path=$goodStderr.path;text=$goodStderr.text.Replace('elements=33','elements=132')}
    if(-not (Get-Beta7FailureGrades @($slateStdout,$slateStderr) @{public=@(33,132)}).f1_pass){throw 'Complete twelve-segment slate transaction fixture failed.'}
    foreach($badElements in @(0,56,74,146)) {
        $badTopology=[pscustomobject]@{path=$goodStdout.path;text=$goodStdout.text.Replace('elements=33',"elements=$badElements")}
        $badTopologyDiagnostic=[pscustomobject]@{path=$goodStderr.path;text=$goodStderr.text.Replace('elements=33',"elements=$badElements")}
        $badTopologyGrade=Get-Beta7FailureGrades @($badTopology,$badTopologyDiagnostic) @{public=@(33,132)}
        if($badTopologyGrade.f1_pass -or @($badTopologyGrade.f1_commit_without_preroll | Where-Object {$_ -match 'Untrusted.*topology'}).Count -eq 0){throw "Out-of-contract topology elements=$badElements incorrectly passed."}
        $topologyGrade=Get-Beta7WorkerTopologyGrade @($badTopology,$badTopologyDiagnostic) @([pscustomobject]@{id='public'}) @([pscustomobject]@{channel_id='public';state='ON_AIR';pid=123}) @([pscustomobject]@{label='public';verdict='PASS'}) @{public=@(33,132)}
        if($topologyGrade.pass){throw "Worker topology grader accepted out-of-contract elements=$badElements."}
    }
    $rotationFixtures=Invoke-R16RotationAwareLogFixtures
    Save-Result @{verdict='PASS';checks=@('Quiescence requires exact STOPPED with no worker PID','Disabled quiescent intent preserves full config','Two-restart barrier helpers are defined','Canonical full-config comparison ignores only auto_start and property order','Canonical full-config comparison rejects nested changes and missing fields','Boundary observations at 10/30/31/301 seconds','Fresh-run grading excludes pre-measurement history','Any WORKER_RESULT and output stall fail','Incomplete reload transaction/firing/diagnostic EOF fails','Complete 33-element programme and 132-element slate transactions pass','Out-of-contract 0/56/74/146 element counts fail','Zero held streams fails','Missing or mismatched finite rebase fails','Firing before preroll fails','Commit before rebase fails','Missing selector handoff or old-tail detach fails','Exact R14 4-to-1 independent log slices pass','Exact R14 missing-preroll, missing-handoff, partial-topology, worker-exit, and no-later-output mutations fail','Exact beta.5 Blackwell partial-pipeline clean-worker-exit log fails','Embargo rows are ignored for airtime overlap','Malformed premiere duration fails','Sixteen serialized transport offsets fit eight hours','Rotation-aware checkpoint collection preserves failure markers and fails closed');rotation_fixtures=$rotationFixtures.checks} 'SELF-TEST.json'
    return
}
. (Join-Path $PSScriptRoot 'Beta7TransportProbeR16.ps1')
function Get-LogOffsets {
    return Get-R16LogCheckpoint -ProgramDataRoot $ProgramDataRoot
}
function Copy-PhaseLogs([string]$Phase, $Checkpoint) {
    return Copy-R16LogsSinceCheckpoint -Checkpoint $Checkpoint -RunRoot $runRoot -Phase $Phase
}
function Invoke-PremeasurementTopologyProof([string]$Phase, $Plan, $Offsets, $ExpectedElements) {
    ...
    foreach($channel in $channels){
        ...
        $allowed=@($ExpectedElements[$channel.id] | ForEach-Object {[int]$_})
        $unexpectedTopology=@($commits | Where-Object {[int]$_ -notin $allowed})
        if(-not $commits.Count -or $unexpectedTopology.Count -or @($commits | Where-Object {[int]$_ -eq 33}).Count -lt 1){throw "$Phase premeasurement topology proof failed for $($channel.id): expected a complete single-programme elements=33 transaction and only bound slate/programme shapes $($allowed -join ',')."}
        ...
        if([string]$state.state -ne 'ON_AIR' -or -not $state.pid -or [string]$state.current_source_label -cne [string]$targetRow.asset_label){throw "$Phase controlled programme transition was not ON_AIR on the expected source for $($channel.id)."}
        $stderrText=[string](@($channelLogs | Where-Object {$_.path -match '(?i)gst-worker\.stderr\.log$'} | ForEach-Object {$_.text}) -join "`n")
        if(-not (Test-Beta7PostCommitOutput $stderrText 33)){throw "$Phase single-programme commit for $($channel.id) has no subsequent output proof."}
        $details += @{channel=$channel.id;transition_target_utc=([datetimeoffset]::Parse([string]$targetRow.scheduled_at).UtcDateTime).ToString('o');expected_source_label=[string]$targetRow.asset_label;observed_source_label=[string]$state.current_source_label;pid=[int]$state.pid;commit_elements=$commits;allowed_elements=$allowed;post_commit_output=$true}
    }
    ...
    foreach($log in $logs){if([string]$log.text -match '(?i)elements=(?:56|74)\b|WORKER_RESULT|output stalled|no output for 10\s*(?:seconds|s)'){throw "$Phase premeasurement logs contain forbidden failure evidence."}}
    $evidence=@{phase=$Phase;profile=$profile;captions_off=$true;caption_environment=$envEvidence;controlled_transitions=$details;captured_at=[datetime]::UtcNow.ToString('o');expected_elements=$ExpectedElements;required_programme_elements=33;failure_grade=$grade}
...
try {
    ...
    foreach ($phase in $phases) {
        ...
        # R16 transport/topology evidence is deliberately captions OFF; caption
        ...
        # The measured eight hours grade only bytes written after the separately
        # passing controlled-transition preflight.
        $offsets=Get-LogOffsets
        # Give the synchronous phase receipt a near-future deadline before the
        # eight-hour clock starts. A missed receipt aborts the phase.
    ...
    if ($owned.Count -and $token) {
        try { Stop-OwnedChannels; Save-Result @{stopped=$owned;at=[datetime]::UtcNow.ToString('o')} 'STOP-VERIFIED.json'; $cleanupVerified=$true }
            ...
            Save-Result @{verdict='FAIL';source_sha=$SourceSha;error=if($primaryFailure){$primaryFailure}else{'cleanup failure'};cleanup_error=$cleanupFailure} 'VERDICT.json'
        }
    }
