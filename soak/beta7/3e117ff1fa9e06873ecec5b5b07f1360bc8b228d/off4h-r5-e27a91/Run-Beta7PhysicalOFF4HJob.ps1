...
$outputRoot=Join-Path $missionRoot 'physical-soak-off4h-r5-e27a91'
...
function Publish-SoakJobReport([switch]$WithArchive){
    ...
    if($WithArchive -and (Test-Path -LiteralPath $outputRoot)){
        ...
        try{
            ...
            $jobLog=Join-Path $missionRoot 'physical-soak-off4h-r5-e27a91-job.log'
...
try{
    ...
    $transcript=Join-Path $missionRoot 'physical-soak-off4h-r5-e27a91-job.log'
    ...
    Write-Beta5JsonAtomic $report (Join-Path $missionRoot 'physical-soak-off4h-r5-e27a91-job.json')
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{Publish-SoakJobReport -WithArchive}
        catch{
            [IO.File]::WriteAllText((Join-Path $missionRoot 'physical-soak-off4h-r5-e27a91-publish-error.txt'),[string]$_.Exception.Message)
            try{Publish-SoakJobReport}catch{[IO.File]::AppendAllText((Join-Path $missionRoot 'physical-soak-off4h-r5-e27a91-publish-error.txt'),"`nStatus publication also failed: "+[string]$_.Exception.Message)}
        }
    }
}
