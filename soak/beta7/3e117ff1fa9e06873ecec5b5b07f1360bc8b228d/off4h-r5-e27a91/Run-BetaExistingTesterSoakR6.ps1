...
function Set-OwnedChannelsQuiescentIntent {
    foreach ($channel in $owned) {
        # Keep the channel enabled so the automation loop continues consuming
        # its command queue, but suppress CA-2's automatic start while this
        # harness intentionally stops it. Round-trip the complete stored
        # config so sink and relay settings are preserved.
        $config = Invoke-Station GET "/api/staff/egress/channels/$channel/config"
        if (-not $config -or -not [bool]$config.enabled) {
            throw "Owned channel $channel must be enabled before it can be quiesced."
        }
        ...
        if (-not (Test-QuiescentChannelConfig $written)) {
            throw "Owned channel $channel did not persist enabled=true, auto_start=false."
        }
        if (-not (Test-ConfigSemanticallyPreserved $originalConfig $written)) {
            throw "Owned channel $channel full config changed during quiescent-intent PUT."
        }
        $readback = Invoke-Station GET "/api/staff/egress/channels/$channel/config"
        if (-not (Test-QuiescentChannelConfig $readback)) {
            throw "Owned channel $channel quiescent-intent readback failed."
        }
        if (-not (Test-ConfigSemanticallyPreserved $originalConfig $readback)) {
            throw "Owned channel $channel full config readback changed after quiescent-intent PUT."
        }
    }
}
function Stop-OwnedChannels {
    Set-OwnedChannelsQuiescentIntent
    # Queue a terminal stop for every channel only after every auto-start
    # suppression readback has passed. This ordering prevents the harness from
    # racing its own stop against CA-2's durable start intent.
    foreach ($channel in $owned) {
        Invoke-Station POST "/api/staff/egress/channels/$channel/commands" @{action='stop'} | Out-Null
    }
    $deadline = [datetime]::UtcNow.AddMinutes(2)
    $quietSince=$null
    $quietWindowSeconds=4.0
    do {
        Start-Sleep -Seconds 2
        $remaining = @()
        foreach ($channel in $owned) {
            $state = Invoke-Station GET "/api/staff/egress/channels/$channel/state"
            if (-not (Test-QuiescentChannelState $state)) {
                $remaining += $channel
            }
        }
        if ($remaining.Count) {
            ...
            elseif (([datetime]::UtcNow-$quietSince).TotalSeconds -ge $quietWindowSeconds) { return }
        }
        # Keep issuing terminal stops throughout both recovery and the quiet
        # confirmation window. This drains a late start from any CA-2 tick
        # already in flight when auto_start was disabled.
        foreach ($channel in $owned) {
            Invoke-Station POST "/api/staff/egress/channels/$channel/commands" @{action='stop'} | Out-Null
        }
...
function Get-Beta7FailureGrades($Logs, $ExpectedElementsPerChannel=@{}) {
    ...
    foreach ($log in @($Logs)) {
        ...
        foreach ($line in @(([string]$log.text -split "`r?`n"))) {
            if ($line -match '(?i)WORKER_RESULT.*error\W*[:=]\s*(?:none|null).*teardown_clean\W*[:=]\s*(?:true|yes)') { $f1 += [string]$line }
...
if ($SelfTest) {
    ...
    $freshGrade = Get-Beta7FailureGrades $freshLogs @{public=@(33)}
    ...
    $postOffsetGrade = Get-Beta7FailureGrades $postOffsetLogs @{public=@(33)}
...
'@}
    $goodGrade=Get-Beta7FailureGrades @($goodStdout,$goodStderr) @{public=@(33)}
...
'@};stderr=$goodStderr},
        ...
        $badGrade=Get-Beta7FailureGrades @($bad.stdout,$bad.stderr) @{public=@(33)}
        if($badGrade.f1_pass){throw "Invalid reload transaction fixture passed: $($bad.name)"}
    }
    Save-Result @{verdict='PASS';checks=@('Quiescence requires exact STOPPED with no worker PID','Quiescent intent requires enabled=true and auto_start=false','Canonical full-config comparison ignores only auto_start and property order','Canonical full-config comparison rejects nested changes and missing fields','Boundary observations at 10/30/31/301 seconds','Fresh-run grading excludes pre-measurement history','Quoted-key clean worker exit and output stall fail','Complete reload transaction passes','Zero held streams fails','Missing or mismatched finite rebase fails','Firing before preroll fails','Commit before rebase fails','Missing selector handoff fails')} 'SELF-TEST.json'
    return
}
    ...
    foreach ($phase in $phases) {
        ...
        if ($RequireTransportAdmission) {
            ...
            foreach ($admissionName in @('acquisition-diagnostic','clean-admission')) {
                ...
                foreach ($channel in $channels) {
                    $warmupSeconds=if($admissionName -eq 'acquisition-diagnostic'){0}else{5}
                    $admissionPending[$channel.id]=Start-Beta7TransportProbe -TspExe $TspExe -Port $channel.port -OutputDirectory (Join-Path $runRoot "$phase/admission/$admissionName") -Label $channel.id -WarmupSeconds $warmupSeconds
