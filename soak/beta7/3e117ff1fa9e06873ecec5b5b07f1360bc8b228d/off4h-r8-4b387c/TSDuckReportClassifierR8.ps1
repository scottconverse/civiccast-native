...
function Get-TSDuckR8RequiredInteger {
...
function Get-TSDuckR8ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR8RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR8RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR8RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR8RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR8MetricsVerdict {
