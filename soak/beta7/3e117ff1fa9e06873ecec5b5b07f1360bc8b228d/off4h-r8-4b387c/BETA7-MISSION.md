...
Status: **R8 ONCE-ONLY PHYSICAL MISSION; BETA.7 REMAINS UNPUBLISHED**. R5 is
...
PowerShell 7 (`pwsh`), which is not installed on `DESKTOP-VBMA6O5`. R8 keeps
...
coordinator-only dual-runtime mode. R8 may run exactly once from its
...
a Git path. R8 uses literal string replacement and executes that exact source
...
- nonce: `BETA7-3E117FF1-OFF4H-R8-4B387C`
...
back and must exactly match the published five-minute R8-owned plan. Final
