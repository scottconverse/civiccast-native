...
$null=& (Join-Path $PackageRoot 'Test-Beta7R8Harness.ps1') -PackageRoot $PackageRoot
if($DryRun){Write-Output 'DRYRUN VERIFIED: exact candidate, R8 harness, and installed-byte adoption plan passed; no mutation performed.';return}
if($env:COMPUTERNAME -ine 'DESKTOP-VBMA6O5'){throw 'This R8 adoption order targets DESKTOP-VBMA6O5 only.'}
...
if(Test-Path -LiteralPath $missionRoot){throw 'R8 mission already exists. Preserve it; never replay this nonce.'}
...
$report=[ordered]@{order='BETA7-R8-ADOPT-VERIFIED-INSTALL';hostname=[string]$env:COMPUTERNAME;candidate_source_sha=[string]$id.candidate_source_sha;started_utc=[datetime]::UtcNow.ToString('o');status='STARTED';step='preflight';error=$null}
try{
    $principal=[Security.Principal.WindowsPrincipal]::new([Security.Principal.WindowsIdentity]::GetCurrent())
    if(-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)){throw 'R8 adoption task must run elevated.'}
    ...
    if($invalidationHash -cne "$($id.r5_invalidation_receipt_sha256)"){throw 'R5 invalidation receipt bytes do not match R8 identity.'}
    ...
    if($LASTEXITCODE -ne 0 -or $directiveCommit -notmatch '^[0-9a-f]{40}$'){throw 'Could not bind R8 harness to the directive commit.'}
    $dirty=@(& git -C $directiveRoot status --porcelain -- $relativePackage)
    if($LASTEXITCODE -ne 0 -or $dirty.Count){throw 'R8 package differs from its directive commit.'}
    ...
    Write-Output 'BETA7_R8_ADOPTION_RESULT_JSON_BEGIN';Write-Output ($report|ConvertTo-Json -Depth 12 -Compress);Write-Output 'BETA7_R8_ADOPTION_RESULT_JSON_END'
}
if($report.status -ne 'PASS'){throw 'R8 exact installed-byte adoption failed; preserve the mission and issue a new nonce after correction.'}
