...
function Get-TSDuckR10RequiredInteger {
...
function Get-TSDuckR10ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR10RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR10RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR10RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR10RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR10MetricsVerdict {
