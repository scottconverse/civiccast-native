...
if($DryRun){Write-Output 'DRYRUN VERIFIED: exact R10 identity, harness, and authoritative completion plan passed; no mutation performed.';return}
if($env:COMPUTERNAME -ine 'DESKTOP-VBMA6O5'){throw 'This R10 soak job targets DESKTOP-VBMA6O5 only.'}
. (Join-Path $PSScriptRoot 'Beta7Tester.Common.ps1')

$missionRoot=[string]$id.mission_state_root
$taskName=[string]$id.task_name
$outputName='physical-soak-off4h-r10-275243'
...
function New-RunLatch {
    ...
    }catch{throw 'R10 nonce has already been consumed or its durable latch could not be created.'}
...
function Disable-OwnTask {
    ...
    if([bool]$observed.Settings.Enabled){throw 'R10 task remained enabled for future launches at entry.'}
...
try{
    ...
    foreach($entry in $id.actual_runtime_file_sha256.PSObject.Properties){
        ...
        if((Get-FileHash -LiteralPath $filePath -Algorithm SHA256).Hash.ToLowerInvariant() -ne "$($entry.Value)"){throw "Installed runtime changed after R10 adoption: $($entry.Name)"}
    }
    ...
    if(Test-Path -LiteralPath $outputRoot){throw 'R10 output already exists; preserve it and do not replay.'}
    if(Test-Path -LiteralPath $returnRepo){throw 'R10 return checkout already exists; preserve it and do not replay.'}
    ...
    if($cloneExit -ne 0){throw "Could not clone the R10 evidence return branch: $($cloneOutput -join ' ')"}
    ...
    Write-Job
    $report.started_commit=Sync-And-Push @($jobRelative) 'test: beta.7 R10 physical soak STARTED'
    ...
    Write-Job
    $report.phase_commit=Sync-And-Push @($jobRelative) 'test: beta.7 R10 measured phase READY'
    ...
    & (Join-Path $PSScriptRoot 'Run-BetaExistingTesterSoakR10.ps1') -BaseUrl 'http://127.0.0.1:8000' -OutputRoot $outputRoot -SourceSha $id.candidate_source_sha -Minutes 240 -Mode OFF -RequireTransportAdmission -ExpectedElementsPerPhase $expectedElements -OnPhaseStarted $phaseReceipt *> $jobLog
    $runs=@(Get-ChildItem -LiteralPath $outputRoot -Directory)
    if($runs.Count -ne 1){throw 'R10 physical output does not contain exactly one run.'}
    $run=$runs[0].FullName
    $report.result=Get-Content -LiteralPath (Join-Path $run 'VERDICT.json') -Raw|ConvertFrom-Json
    if("$($report.result.verdict)" -ne 'TIMING_LIFETIME_TRANSPORT_PASS'){throw 'R10 physical checks did not pass.'}
    if(-not(Test-Path -LiteralPath (Join-Path $run 'STOP-VERIFIED.json') -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $run 'STOP-FAILED.json') -PathType Leaf) -or -not(Test-Path -LiteralPath (Join-Path $run 'SCHEDULE-CLEANUP-VERIFIED.json') -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $run 'SCHEDULE-CLEANUP-FAILED.json') -PathType Leaf)){throw 'R10 terminal channel and owned-schedule cleanup is not verified.'}
    ...
    Write-Job;$report.evidence_commit=Sync-And-Push @($jobRelative,$archiveRelative) 'test: beta.7 R10 collection and evidence uploaded';$evidencePushed=$true
    ...
    $report.completion_commit=Sync-And-Push @($jobRelative,$completionRelative) 'test: beta.7 R10 authoritative PASS completion'
    ...
    if(Test-Path -LiteralPath (Join-Path $returnRepo '.git')){
        try{
            ...
            $report.failure_commit=Sync-And-Push $failurePaths 'test: beta.7 R10 physical soak FAIL'
...
if($report.job_state -ne 'PASS'){throw "R10 physical soak did not pass: $finalFailure"}
