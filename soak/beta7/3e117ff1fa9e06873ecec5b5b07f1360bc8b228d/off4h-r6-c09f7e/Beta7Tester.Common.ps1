...
function Get-Beta5Identity {
    ...
    if ([int] $id.state_sample_interval_seconds -ne 10 -or [int] $id.caption_status_interval_seconds -ne 30 -or [int] $id.hardware_sample_interval_seconds -ne 30 -or [int] $id.transport_round_interval_seconds -ne 1800) { throw 'R6 sampling intervals are not the reviewed 10/30/30/1800-second contract.' }
    $roundOffsets=@($id.transport_round_offsets_seconds | ForEach-Object {[int]$_})
    if($roundOffsets.Count -ne 8 -or ($roundOffsets -join ',') -cne '0,1800,3600,5400,7200,9000,10800,12600'){throw 'R6 transport offsets are not the reviewed eight-round four-hour cadence.'}
