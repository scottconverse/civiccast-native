...
$missionRoot='C:\CivicCastSoak\missions\beta7-3e117ff1-off4h-r6-c09f7e'
...
$null=& (Join-Path $installedBin 'Test-Beta7R6Harness.ps1') -PackageRoot $installedBin
...
$null=& (Join-Path $bin 'Test-Beta7R6Harness.ps1') -PackageRoot $bin
...
@{order='BETA7-3E117FF1-PHYSICAL-OFF4H-R6';mission_nonce=[string]$id.mission_nonce;task_name=$taskName;launch_requested_utc=[datetime]::UtcNow.ToString('o');candidate_source_sha=[string]$id.candidate_source_sha;job_report=([string]$id.evidence_relative_root+'/job.json');status='LAUNCH_REQUESTED';measured_soak_started=$false} | ConvertTo-Json -Compress
