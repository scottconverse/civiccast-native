...
function Get-TSDuckR12RequiredInteger {
...
function Get-TSDuckR12ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR12RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR12RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR12RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR12RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR12MetricsVerdict {
