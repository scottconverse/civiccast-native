...
Status: **R12 ONCE-ONLY PHYSICAL MISSION; BETA.7 REMAINS UNPUBLISHED**. R5 is
...
PowerShell 7 (`pwsh`), which is not installed on `DESKTOP-VBMA6O5`. R12 keeps
...
coordinator-only dual-runtime mode. R12 may run exactly once from its
...
a Git path. R12 uses literal string replacement and executes that exact source
...
- nonce: `BETA7-3E117FF1-OFF4H-R12-DB2617`
...
back and must exactly match the published five-minute R12-owned plan. Final
