...
<#
Non-mutating preflight for the beta.7 R12 physical-soak harness.
...
function Invoke-TransportFixtures {
    ...
    $transport = Join-Path $Root 'Beta7TransportProbeR12.ps1'
    . $transport
...
function Invoke-GitStderrFixture {
    ...
    $repo=Join-Path ([IO.Path]::GetTempPath()) ('civiccast-r12-git-' + [guid]::NewGuid().ToString('N'))
    try{
        ...
        $null=Invoke-Beta5Git -Repository $repo -Arguments @('config','user.name','R12 fixture')
        $null=Invoke-Beta5Git -Repository $repo -Arguments @('config','user.email','r12-fixture@civiccast.invalid')
...
function Invoke-ChildPreflight {
    ...
    $expected = @(
        ...
        'Beta7TransportProbeR12.ps1',
        'Invoke-Beta7TesterUpgrade.ps1',
        'Run-Beta7PhysicalOFF4HJob.ps1',
        'Run-BetaExistingTesterSoakR12.ps1',
        'Start-Beta7PhysicalOFF4HJob.ps1',
        'Test-Beta7HarnessPackage.ps1',
        'Test-Beta7R12Harness.ps1',
        'TSDuckReportClassifierR12.ps1'
    )
    foreach ($name in $expected) {
        Assert-Check (Test-Path -LiteralPath (Join-Path $root $name) -PathType Leaf) "Required R12 harness test or executable is missing: $name"
    }
    $scripts = @(Get-ChildItem -LiteralPath $root -Recurse -File -Filter '*.ps1')
    Assert-Check ($scripts.Count -ge $expected.Count) 'R12 package does not contain every required PowerShell file.'
    ...
    foreach($liveCallerName in @('Invoke-Beta7TesterUpgrade.ps1','Start-Beta7PhysicalOFF4HJob.ps1')){
        ...
        Assert-Check ($liveCallerText -notmatch '(?s)Test-Beta7R12Harness\.ps1.{0,200}-RequireDualRuntime') "Tester-live caller incorrectly requires unavailable pwsh: $liveCallerName"
    }

    $identityPath = Join-Path $root 'run-identity.json'
    Assert-Check (Test-Path -LiteralPath $identityPath -PathType Leaf) 'R12 run identity is missing.'
    ...
    Assert-Check ($null -ne $packageProof -and "$($packageProof.manifest_sha256)" -ceq "$($identity.expected_harness_manifest_sha256)") 'R12 package proof did not bind the exact manifest hash.'
    $boundIdentity = & (Join-Path $root 'Assert-Beta7CandidateBinding.ps1') -IdentityPath $identityPath
    Assert-Check ($null -ne $boundIdentity -and "$($boundIdentity.mission_nonce)" -ceq 'BETA7-3E117FF1-OFF4H-R12-DB2617') 'R12 candidate binding proof did not return the exact mission.'
    $offsets = @($identity.transport_round_offsets_seconds | ForEach-Object { [int]$_ })
    Assert-Check (($offsets -join ',') -eq '0,1800,3600,5400,7200,9000,10800,12600') 'R12 identity does not bind exactly eight 30-minute transport offsets.'
    Assert-Check (@($offsets | Where-Object { $_ -lt 0 -or $_ -ge 14400 -or $_ % 1800 -ne 0 }).Count -eq 0) 'R12 transport offsets do not fit the measured four-hour cadence.'

    $transportText = Get-Content -LiteralPath (Join-Path $root 'Beta7TransportProbeR12.ps1') -Raw
    Assert-Check ($transportText -notmatch "(?i)'-P'\s*,\s*'skip'") 'R12 transport probe still invokes the unavailable skip plugin.'
    Assert-Check ($transportText -match [regex]::Escape("@('-P','until','--seconds','30')")) 'R12 transport probe does not bind a 30-second until stage.'
    Assert-Check ($transportText -match [regex]::Escape("@('-P','analyze','--json','--output-file',`$quotedReport,'-O','drop')")) 'R12 transport probe does not bind analyze JSON output and drop.'

    $driver = Join-Path $root 'Run-BetaExistingTesterSoakR12.ps1'
    $driverText = Get-Content -LiteralPath $driver -Raw
    Assert-Check ($driverText -match [regex]::Escape("@('CIVICCAST_CAPTION_TAP','CIVICCAST_CAPTION_TAP_DIR')")) 'R12 caption-environment evidence does not use the product caption-tap variable names.'
    Assert-Check ($driverText -notmatch 'CIVICAST_CAPTION_TAP') 'R12 driver contains the misspelled caption-tap environment prefix.'
    ...
    Assert-Check ($driverText -match $scheduleCleanupGuard) 'R12 finalizer must invoke exact-notes schedule discovery behind the exact token-only guard, independent of recorded IDs.'
    ...
    Assert-Check (@($driverErrors).Count -eq 0) 'R12 driver could not be parsed for disabled-channel recovery.'
    foreach($functionName in @('Test-QuiescentChannelConfig','Test-JsonValueEqual','Test-ConfigPreservedForBarrier','Set-OwnedChannelsQuiescentIntent')) {
        ...
        Assert-Check ($definition.Count -eq 1) "R12 driver must define exactly one $functionName function."
    ...
    function Invoke-Station([string]$Method,[string]$Path,$Body=$null,[int]$Timeout=120) {
        $script:configCalls++
        if($Path -cne '/api/staff/egress/channels/public/config'){throw "Unexpected R12 recovery fixture path: $Path"}
        if($Method -ceq 'PUT'){$script:storedConfig=$Body|ConvertTo-Json -Depth 20 -Compress|ConvertFrom-Json}
        elseif($Method -cne 'GET'){throw "Unexpected R12 recovery fixture method: $Method"}
        return $script:storedConfig|ConvertTo-Json -Depth 20 -Compress|ConvertFrom-Json
    }
    Set-OwnedChannelsQuiescentIntent
    Assert-Check ($script:configCalls -eq 3 -and [bool]$script:storedConfig.enabled -and -not [bool]$script:storedConfig.auto_start) 'Production R12 recovery did not restore the R11-disabled channel with auto-start suppressed.'
    Assert-Check ([string]$script:storedConfig.sinks[0].uri -ceq 'udp://127.0.0.1:9001' -and [int]$script:storedConfig.sinks[0].latency_ms -eq 2000) 'Production R12 recovery changed the existing sink config.'
    Assert-Check ($barrierConfigs.ContainsKey('public') -and [bool]$barrierConfigs.public.enabled -and -not [bool]$barrierConfigs.public.auto_start) 'Production R12 recovery did not retain the verified enabled config for final checks.'
    ...
    Assert-Check ($unsafeRejected -and $script:configCalls -eq 1 -and -not $barrierConfigs.Count) 'Production R12 recovery accepted unsafe disabled-plus-auto-start intent.'
    Assert-Check ($driverText -notmatch 'Restart-Service|disabled-auto-start barrier|restored-config barrier') 'R12 still uses the R11 supervisor-restart barrier that timed out on the tester.'
    ...
    ) -Message 'R12 does not use the proven enabled/auto-start-off stop path with bounded repeat commands.'
    Assert-ContainsInOrder -Text $driverText -Needles @(
    ...
    ) -Message 'R12 can still discard the primary failure when cleanup also fails.'
    ...
    ) -Message 'R12 startup, admission, topology proof, receipt, and measured-clock order is not exact.'
    Assert-ContainsInOrder -Text $driverText -Needles @(
    ...
    ) -Message 'R12 premeasurement proof no longer requires a future controlled transition with complete elements=146 transaction evidence.'
    ...
    Assert-Check (@($upgradeErrors).Count -eq 0) 'R12 adoption script could not be parsed for its actual relative-package assignment.'
    $relativeAssignments=@($upgradeAst.FindAll({param($node)
        $node -is [System.Management.Automation.Language.AssignmentStatementAst] -and
        $node.Left.Extent.Text -ceq '$relativePackage'
    },$true))
    Assert-Check ($relativeAssignments.Count -eq 1) 'R12 adoption must define exactly one relative-package assignment.'
    $directiveRoot='C:\fixture'
    $PackageRoot='C:\fixture\soak\beta7\candidate\off4h-r12-db2617'
    $relativePackage=$null
    . ([scriptblock]::Create($relativeAssignments[0].Extent.Text))
    Assert-Check ($relativePackage -ceq 'soak/beta7/candidate/off4h-r12-db2617') 'Actual R12 adoption path conversion did not produce the exact Git path.'
    Assert-Check ($upgradeText -match '(?s)\$localBlob\s*=\s*\(Invoke-Beta5Git.{0,300}''rev-parse''.{0,100}:\$relativeInvalidation') 'R12 adoption does not compare the normalized local invalidation index blob.'
    Assert-Check ($upgradeText -notmatch '(?s)\$localBlob\s*=.{0,300}''hash-object''.{0,100}''--no-filters''') 'R12 adoption still compares raw CRLF invalidation bytes with a normalized remote blob.'
    $selfTestRoot = Join-Path ([IO.Path]::GetTempPath()) ('civiccast-r12-harness-' + [guid]::NewGuid().ToString('N'))
    ...
    Assert-Check (@($jobErrors).Count -eq 0) 'R12 job could not be parsed for its actual terminal-state function.'
    $evidenceRelativeAssignments=@($jobAst.FindAll({param($node)
        $node -is [System.Management.Automation.Language.AssignmentStatementAst] -and
        $node.Left.Extent.Text -ceq '$relative'
    },$true))
    Assert-Check ($evidenceRelativeAssignments.Count -eq 1) 'R12 job must define exactly one evidence-entry relative-path assignment.'
    ...
    Assert-Check ($relative -ceq 'run/OFF/states.ndjson') 'Actual R12 evidence-entry path conversion did not produce the exact ZIP path.'
    $disableFunctions=@($jobAst.FindAll({param($node)
        $node -is [System.Management.Automation.Language.FunctionDefinitionAst] -and
        $node.Name -ceq 'Disable-OwnTask'
    },$true))
    Assert-Check ($disableFunctions.Count -eq 1) 'R12 job must define exactly one Disable-OwnTask function.'
    . ([scriptblock]::Create($disableFunctions[0].Extent.Text))
    $taskName='CivicCast-R12-Fixture'
    $report=[ordered]@{}
    ...
    ) 'Actual R12 task-disable function rejected the valid running-but-disabled Windows task state.'
    $phaseAssignments=@($jobAst.FindAll({param($node)
        $node -is [System.Management.Automation.Language.AssignmentStatementAst] -and
        $node.Left.Extent.Text -ceq '$phaseReceipt'
    },$true))
    Assert-Check ($phaseAssignments.Count -eq 1) 'R12 job must define exactly one physical phase receipt callback.'
    ...
    ) 'Actual R12 physical phase callback did not return one exact verified acknowledgement.'
    $fixtureRemote=('b' * 40)
    $mismatchRejected=$false
    try{$null=& $phaseCallback 'OFF' $fixtureBegin $fixtureEnd}catch{$mismatchRejected=$true}
    Assert-Check $mismatchRejected 'Actual R12 physical phase callback accepted a remote commit mismatch.'
    ...
    Assert-Check ($stateFunction.Count -eq 1) 'R12 job must define exactly one Get-AuthoritativeState function.'
    . ([scriptblock]::Create($stateFunction[0].Extent.Text))
    Assert-Check ((Get-AuthoritativeState $false $false $false $false $false $false) -eq 'FAIL') 'Actual R12 state function accepted failed collection/cleanup.'
    Assert-Check ((Get-AuthoritativeState $true $true $true $false $false $false) -eq 'EVIDENCE_PUBLISH_FAIL') 'Actual R12 state function accepted missing evidence publication.'
    Assert-Check ((Get-AuthoritativeState $true $true $true $true $true $false) -eq 'EVIDENCE_PUBLISH_FAIL') 'Actual R12 state function accepted missing completion publication.'
    Assert-Check ((Get-AuthoritativeState $true $true $true $true $true $true) -eq 'PASS') 'Actual R12 state function rejected the complete authoritative path.'
    Assert-ContainsInOrder -Text $jobText -Needles @(
        ...
        '$phaseReceipt='
    ) -Message 'R12 job can signal startup before the remote STARTED blob is verified.'
    Assert-ContainsInOrder -Text $jobText -Needles @(
        '$phaseReceipt={',
        'Write-Job',
    ...
    ) -Message 'R12 physical phase callback does not return an exact remotely verified acknowledgement.'
    Assert-ContainsInOrder -Text $jobText -Needles @(
        "STOP-VERIFIED.json",
        "STOP-FAILED.json",
        "SCHEDULE-CLEANUP-VERIFIED.json",
        "SCHEDULE-CLEANUP-FAILED.json",
        "throw 'R12 terminal channel and owned-schedule cleanup is not verified.'"
    ) -Message 'R12 job does not fail closed on terminal channel and schedule cleanup.'
    Assert-ContainsInOrder -Text $jobText -Needles @(
    ...
    ) -Message 'R12 job can mark authoritative PASS before verified evidence and completion publication.'
    Assert-ContainsInOrder -Text $jobText -Needles @(
        '$r5InvalidationPath=',
    ...
    ) -Message 'R12 job does not bind overlap replacement to the exact invalidated R5 plan.'
    Assert-Check ($jobText -match [regex]::Escape("'EVIDENCE_PUBLISH_FAIL'")) 'R12 job lacks an evidence-publication failure state.'
    Assert-Check ($jobText -match [regex]::Escape("if(`$report.job_state -ne 'PASS'){throw")) 'R12 job can return success for a non-PASS job state.'
    Assert-Check ($jobText -match '(?i)completion(?:_relative|Path|\.json)') 'R12 job lacks a separate completion receipt path.'
    Assert-Check ($jobText -match '(?i)verified_evidence_commit') 'R12 completion receipt does not reference verified evidence.'
    Assert-Check ($jobText -match '(?i)evidence_sha256') 'R12 completion receipt does not bind evidence bytes.'

    $jobPass = [pscustomobject]@{ job_state = 'PASS'; evidence_relative_path = 'soak/r12/evidence.zip' }
    $receiptPass = [pscustomobject]@{ status = 'PASS'; verified_evidence_commit = 'abc123'; evidence_sha256 = ('a' * 64); evidence_relative_path = 'soak/r12/evidence.zip' }
    Assert-Check (Test-AuthoritativePassState $jobPass $receiptPass) 'Authoritative completion fixture did not pass.'
    foreach ($fault in @(
        [pscustomobject]@{ job_state = 'FAIL'; evidence_relative_path = 'soak/r12/evidence.zip' },
        [pscustomobject]@{ job_state = 'EVIDENCE_PUBLISH_FAIL'; evidence_relative_path = 'soak/r12/evidence.zip' },
        [pscustomobject]@{ job_state = 'PASS'; evidence_relative_path = 'soak/r12/evidence.zip' }
    )) {
        $receipt = if ($fault.job_state -eq 'PASS') { [pscustomobject]@{ status = 'FAIL'; verified_evidence_commit = $null; evidence_sha256 = $null; evidence_relative_path = 'soak/r12/evidence.zip' } } else { $receiptPass }
...
if ($RequireDualRuntime) {
    ...
    foreach ($psHostExe in @($windowsPowerShell, $pwsh)) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "R12 harness child preflight failed in $psHostExe with exit code $LASTEXITCODE." }
    }
    Write-Output 'R12 harness preflight PASS in Windows PowerShell 5.1 and pwsh 7.'
    return
}

$result = Invoke-ChildPreflight -Root $resolvedRoot
$result | ConvertTo-Json -Depth 6 -Compress
Write-Output "R12 harness preflight PASS in current runtime $($PSVersionTable.PSEdition) $($PSVersionTable.PSVersion)."
