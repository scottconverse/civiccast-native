...
function Get-Beta5Identity {
    ...
    if ([int] $id.state_sample_interval_seconds -ne 10 -or [int] $id.caption_status_interval_seconds -ne 30 -or [int] $id.hardware_sample_interval_seconds -ne 30 -or [int] $id.transport_round_interval_seconds -ne 1800) { throw 'R12 sampling intervals are not the reviewed 10/30/30/1800-second contract.' }
    $roundOffsets=@($id.transport_round_offsets_seconds | ForEach-Object {[int]$_})
    if($roundOffsets.Count -ne 8 -or ($roundOffsets -join ',') -cne '0,1800,3600,5400,7200,9000,10800,12600'){throw 'R12 transport offsets are not the reviewed eight-round four-hour cadence.'}
...
if("$($id.r5_invalidation_receipt_sha256)" -notmatch '^[0-9a-f]{64}$' -or "$($id.r5_invalidation_receipt_sha256)" -ceq ('0' * 64) -or "$($id.r5_invalidation_remote_commit)" -notmatch '^[0-9a-f]{40}$' -or "$($id.r5_invalidation_remote_commit)" -ceq ('0' * 40)){throw 'R5 invalidation provenance is absent, zero, or invalid.'}
    ...
    foreach ($phase in $expectedPhases) {
        ...
        foreach ($topology in $expectedTopology) {
            ...
            if($counts.Count -ne 1 -or [int]$counts[0] -ne 146){throw "Identity topology expectation for $($phase.Name)/$($topology.Name) must be exactly the reviewed current-run preflight target 146."}
        }
    }
    $failedCounts=@($id.topology_provenance.known_failed_elements|ForEach-Object{[int]$_}|Sort-Object -Unique)
    if(($failedCounts -join ',') -cne '33,56,74'){throw 'Topology provenance must bind all known failed counts 33, 56, and 74.'}
