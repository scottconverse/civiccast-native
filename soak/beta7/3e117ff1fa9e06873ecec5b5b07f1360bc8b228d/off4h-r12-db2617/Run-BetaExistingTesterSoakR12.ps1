...
function Get-R12OwnedActiveScheduleRows($Rows, [string]$OwnedNotes) {
...
function Remove-RemainingOwnedSchedule {
    ...
    foreach($channel in $channels) {
        ...
        $discovered += @(Get-R12OwnedActiveScheduleRows $rows $ownedNotes | ForEach-Object {
            if(-not $_.id){throw "R12-owned active schedule row on $($channel.id) has no ID."}
    ...
    if($duplicateIds.Count){throw "R12-owned active schedule IDs are duplicated across channel queries: $($duplicateIds.Name -join ',')."}
    ...
    foreach($channel in $channels){
        $rows=@(Invoke-Station GET "/api/staff/schedule?channel_id=$($channel.id)")
        $remaining += @(Get-R12OwnedActiveScheduleRows $rows $ownedNotes)
    }
    if($remaining.Count){throw "$($remaining.Count) active R12-owned schedule rows remain after cleanup."}
...
'@
    ...
    $cleanupSelected=@(Get-R12OwnedActiveScheduleRows $cleanupFixtures $ownedNotes)
...
. (Join-Path $PSScriptRoot 'Beta7TransportProbeR12.ps1')
...
try {
    ...
    foreach ($phase in $phases) {
        ...
        # R12 transport/topology evidence is deliberately captions OFF; caption
