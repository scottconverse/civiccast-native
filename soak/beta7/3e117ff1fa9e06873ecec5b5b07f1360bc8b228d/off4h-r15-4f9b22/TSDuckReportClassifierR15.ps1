...
function Get-TSDuckR15RequiredInteger {
...
function Get-TSDuckR15ReportMetrics {
    ...
    foreach ($pidRow in $pidRows) {
        ...
        $discontinuities += Get-TSDuckR15RequiredInteger -Object $pidRow.packets -Name 'discontinuities'
    }
    return [ordered]@{
        packets_total = Get-TSDuckR15RequiredInteger -Object $packets -Name 'total'
        invalid_syncs = Get-TSDuckR15RequiredInteger -Object $packets -Name 'invalid-syncs'
        transport_errors = Get-TSDuckR15RequiredInteger -Object $packets -Name 'transport-errors'
        # TSDuck's current JSON has no ts-level/PCR-only discontinuity counter.
        pid_discontinuities = $discontinuities
    }
}

function Get-TSDuckR15MetricsVerdict {
