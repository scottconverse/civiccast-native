...
function Get-R15OwnedActiveScheduleRows($Rows, [string]$OwnedNotes) {
...
function Remove-RemainingOwnedSchedule {
    ...
    foreach($channel in $channels) {
        ...
        $discovered += @(Get-R15OwnedActiveScheduleRows $rows $ownedNotes | ForEach-Object {
            if(-not $_.id){throw "R15-owned active schedule row on $($channel.id) has no ID."}
    ...
    if($duplicateIds.Count){throw "R15-owned active schedule IDs are duplicated across channel queries: $($duplicateIds.Name -join ',')."}
    ...
    foreach($channel in $channels){
        $rows=@(Invoke-Station GET "/api/staff/schedule?channel_id=$($channel.id)")
        $remaining += @(Get-R15OwnedActiveScheduleRows $rows $ownedNotes)
    }
    if($remaining.Count){throw "$($remaining.Count) active R15-owned schedule rows remain after cleanup."}
...
'@
    ...
    $cleanupSelected=@(Get-R15OwnedActiveScheduleRows $cleanupFixtures $ownedNotes)
...
try {
    ...
    foreach ($phase in $phases) {
        ...
        # R15 transport/topology evidence is deliberately captions OFF; caption
