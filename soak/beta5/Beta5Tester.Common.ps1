...
function Get-Beta5Identity {
    ...
    if ("$($id.expected_version)" -notmatch '^1\.0\.0-beta\.5(?:[.+-][0-9A-Za-z.-]+)?$') { throw 'Expected version is not beta.5.' }
    if ("$($id.kit_base_url)" -notlike "*/$($id.candidate_source_sha)/") { throw 'Kit URL is not candidate-bound.' }
