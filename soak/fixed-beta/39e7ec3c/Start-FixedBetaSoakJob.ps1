...
$files=@('Run-FixedBetaSoakJob.ps1','Run-BetaExistingTesterSoak.ps1','FixedBetaTransportProbe.ps1','TSDuckReportClassifier.ps1')
...
if($DryRun){Write-Output 'DRYRUN: copy four soak scripts; create one dedicated six-hour task; start it once; preserve coordination tasks. No actions performed.';return}
...
$taskName='CivicCast-FixedBeta-39e7-PhysicalSoak-R1'
foreach($installedHelper in @($identityPath,(Join-Path $bin 'Beta5Tester.Common.ps1'))){if(-not(Test-Path -LiteralPath $installedHelper -PathType Leaf)){throw "Verified upgrade helper is missing: $installedHelper"}}
...
if(Test-Path -LiteralPath (Join-Path $missionRoot 'physical-soak')){throw 'Physical soak evidence already exists; preserve it.'}
...
@{order='FIXED-BETA-PHYSICAL-SOAK-R1';task_name=$taskName;launch_requested_utc=[datetime]::UtcNow.ToString('o');candidate_source_sha='39e7ec3cbb4ccbeb3009ff3257dfc314010151f3';job_report='soak/fixed-beta-39e7/physical-soak-r1/job.json';status='LAUNCH_REQUESTED';measured_soak_started=$false} | ConvertTo-Json -Compress
