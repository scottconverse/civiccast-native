...
$sourceFiles=@('Beta5Tester.Common.ps1','AUTORUN-FIXED-BETA6-FETCH-INSTALL.ps1','run-identity.json')
...
$report=[ordered]@{order='FIXED-BETA6-UPGRADE-R1';hostname=[string]$env:COMPUTERNAME;candidate_source_sha=[string]$id.candidate_source_sha;started_utc=[datetime]::UtcNow.ToString('o');status='STARTED';step='preflight';legacy_tasks=@();identity=$null;error=$null}
try{
    ...
    if(-not(Test-Path -LiteralPath 'C:\CivicCastHostStore\install' -PathType Container)){throw 'Expected existing tester install root is absent.'}
    ...
    & (Join-Path $bin 'AUTORUN-FIXED-BETA6-FETCH-INSTALL.ps1') -IdentityPath $identityPath
