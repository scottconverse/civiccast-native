...
$returnRepo=Join-Path $missionRoot 'return-repo-r3'
$relativeRoot='soak/fixed-beta-39e7/physical-soak-r3'
...
try{
    ...
    $previousRuns=@(Get-ChildItem -LiteralPath (Join-Path $missionRoot 'physical-soak-r2') -Directory)
    if($previousRuns.Count -ne 1){throw 'Expected exactly one preserved R2 schedule evidence directory.'}
    ...
    & (Join-Path $PSScriptRoot 'Run-BetaExistingTesterSoak.ps1') -BaseUrl 'http://127.0.0.1:8000' -OutputRoot $outputRoot -SourceSha $id.candidate_source_sha -Minutes 120 -Mode Both -ExistingRunRoot $previousRuns[0].FullName -OnPhaseStarted $phaseReceipt *> $transcript
