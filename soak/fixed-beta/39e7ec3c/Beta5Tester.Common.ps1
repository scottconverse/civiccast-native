...
function Get-Beta5Identity {
    ...
    $required = @(
        ...
        'mission_state_root', 'return_repo_path', 'planned_seconds',
        'sample_interval_seconds', 'channels'
    )
    ...
    foreach ($name in 'candidate_source_sha', 'build_source_sha', 'gate_a_source_sha') {
        if ("$($id.$name)" -notmatch '^[0-9a-f]{40}$') { throw "Invalid $name." }
    }
    ...
    foreach ($name in 'expected_manifest_sha256', 'expected_installer_sha256') {
        if ("$($id.$name)" -notmatch '^[0-9a-f]{64}$') { throw "Invalid $name." }
    }
    if ("$($id.build_conclusion)" -cne 'success') { throw 'Candidate build conclusion is not exactly success.' }
    if ("$($id.gate_a_verdict)" -cnotin @('PASS', 'PENDING')) { throw 'Gate A verdict must truthfully be exactly PASS or PENDING.' }
    if ("$($id.build_run_id)" -notmatch '^\d+$' -or [int64] $id.build_run_id -le 0 -or "$($id.gate_a_run_id)" -notmatch '^\d+$' -or [int64] $id.gate_a_run_id -le 0) {
    ...
    $isLegacyBeta5 = $version -match '^1\.0\.0-beta\.5(?:[.+-][0-9A-Za-z.-]+)?$'
    $isFixedBeta6 = $candidate -eq '39e7ec3cbb4ccbeb3009ff3257dfc314010151f3' -and
        $version -match '^1\.0\.0-beta\.6(?:[.+-][0-9A-Za-z.-]+)?$'
    if (-not $isLegacyBeta5 -and -not $isFixedBeta6) { throw 'Expected version/candidate is not an accepted beta.5 or fixed beta.6 contract.' }
    if ($isLegacyBeta5 -and $kitBase -notlike "*/$($id.candidate_source_sha)/") { throw 'Legacy beta.5 kit URL is not candidate-bound.' }
    if ($isFixedBeta6 -and $kitBase -cne 'http://192.168.0.135:8766/') { throw 'Fixed beta.6 kit URL is not the approved exact LAN root.' }
...
function Publish-Beta5Report {
    ...
    $allowedRoot = Join-Path $repository ("soak\beta5\" + $Identity.mission)
