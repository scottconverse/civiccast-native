...
function Get-FixedBetaTransportGrade {
...
function Start-FixedBetaTransportProbe {
...
function Complete-FixedBetaTransportProbe {
    ...
    return Get-FixedBetaTransportGrade -Json $json -Label $Probe.Label -Port ([int]$Probe.Port) -ExitCode $exitCode
}

function Stop-FixedBetaTransportProbe {
    param([Parameter(Mandatory)]$Probe)
    $process = $Probe.Process
    $process.Refresh()
