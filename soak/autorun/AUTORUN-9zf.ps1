# AUTORUN-9zf (soak8-e1acfe6) -- read-only: which civiccast code is actually installed after the
...
$out   = @("# AUTORUN-9zf installed-code check after the CLEAN install of 609273d (AUTORUN-9za)", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
...
if ($pack) {
  ...
  try {
    ...
    foreach ($e in $hits) {
      $tmp = Join-Path $env:TEMP ("9zf-" + [IO.Path]::GetFileName($e.FullName))
...
Set-Content "$repo\soak\DIAG-9zf-$stamp.md" -Value ($out -join "`n") -Encoding utf8
git -C $repo add "soak/DIAG-9zf-$stamp.md"
git -C $repo commit --quiet -m "test: autorun-9zf installed-code check $stamp soak8-e1acfe6"
