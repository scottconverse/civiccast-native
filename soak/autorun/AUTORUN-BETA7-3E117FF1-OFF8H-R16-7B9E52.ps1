...
$package = Join-Path $PSScriptRoot '..\beta7\3e117ff1fa9e06873ecec5b5b07f1360bc8b228d\off8h-r16-7b9e52'
$identityPath = Join-Path $package 'run-identity.json'
$expectedTask = 'CivicCast-Beta7-3e117ff1-OFF8H-R16-7B9E52'
$expectedMission = 'BETA7-3E117FF1-OFF8H-R16-7B9E52'
$expectedSource = '3e117ff1fa9e06873ecec5b5b07f1360bc8b228d'
$jobRelative = 'soak/beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d/physical-soak-off8h-r16-7b9e52/job.json'
$terminalJobName = 'physical-soak-off8h-r16-7b9e52-job.json'
...
function Get-VerifiedStartReceipt($Identity) {
    ...
    if ([string]$receipt.schema -cne 'civiccast-beta7-start-verified-v1' -or
        ...
        throw 'R16 start marker does not bind the exact remotely verified candidate receipt.'
    }
    return $receipt
}
...
if ($DryRun) {
    ...
    if (@($parseErrors).Count) { throw "R16 activation order has parse errors: $($parseErrors.Message -join '; ')" }
    ...
    if ($gitCommands.Count) { throw 'R16 launcher must not race the physical job for its Git checkout.' }
    if ($selfText -notmatch '(?s)Start-Beta7PhysicalOFF8HJob.*?AddMinutes\(5\).*?Get-VerifiedStartReceipt.*?terminalJobPath.*?did not publish and remotely verify its start receipt') {
        throw 'R16 launcher does not wait fail-closed for the physical job start-verification marker.'
    }
    if ($selfText -notmatch '(?s)Invoke-Beta7TesterUpgrade\.ps1.*?installedIdentityPath.*?Assert-Beta7CandidateBinding\.ps1.*?Start-Beta7PhysicalOFF8HJob\.ps1') {
        throw 'R16 launcher does not reload the adoption-updated installed identity before launch.'
    }
...
if ($env:COMPUTERNAME -ine 'DESKTOP-VBMA6O5') { throw 'This beta.7 R16 mission targets DESKTOP-VBMA6O5 only.' }
...
$launch = @(& (Join-Path $package 'Start-Beta7PhysicalOFF8HJob.ps1') -PackageRoot $package)
...
do {
    ...
    if (Test-Path -LiteralPath $terminalJobPath -PathType Leaf) {
        ...
        throw "R16 physical task terminated before its remote start receipt: state=$($terminal.job_state), error=$($terminal.error)"
    }
    Start-Sleep -Seconds 2
} while ([datetime]::UtcNow -lt $deadline)
if (-not $verifiedStart) { throw 'R16 physical task did not publish and remotely verify its start receipt within five minutes.' }
