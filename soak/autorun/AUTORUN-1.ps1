# AUTORUN-1 (soak8-e1acfe6) -- fetch + verify the kit, then install it silently.
...
function Reset-AutorunMarker {
  Remove-Item "$root\state\autorun-done\AUTORUN-1.ps1.done" -Force -ErrorAction SilentlyContinue
}
...
if (-not (Test-Path "$dst\SHA256SUMS.txt") -or (Get-Item "$dst\SHA256SUMS.txt").Length -eq 0) {
  ...
  git -C $repo commit --quiet -m "test: autorun-1 kit fetch BLOCKED $stamp soak8-e1acfe6"
...
foreach ($line in $manifest) {
  $h, $rel = $line -split '\s+', 2
  if (-not $rel) { continue }
  $f = Join-Path $dst ($rel.Trim() -replace '/', '\')
...
if ($bad -ne 0) {
  ...
  Set-Content "$repo\soak\INSTALL-RESULT.md" -Value ($log -join "`n") -Encoding utf8
  git -C $repo add soak/INSTALL-RESULT.md
  git -C $repo commit --quiet -m "test: autorun-1 kit verify FAILED bad=$bad $stamp soak8-e1acfe6"
  git -C $repo push --quiet origin $br 2>&1 | Out-Null
  exit 2
}

# ------------------------------------------------- 2. locate the installer exe
$exe = Get-ChildItem $dst -Filter '*setup.exe' -File | Select-Object -First 1
if (-not $exe) { $exe = Get-ChildItem $dst -Filter '*.exe' -File | Select-Object -First 1 }
if (-not $exe) {
  $log += "no installer .exe found at the kit root"
  Set-Content "$repo\soak\INSTALL-RESULT.md" -Value ($log -join "`n") -Encoding utf8
  git -C $repo add soak/INSTALL-RESULT.md
  git -C $repo commit --quiet -m "test: autorun-1 no installer $stamp soak8-e1acfe6"
  git -C $repo push --quiet origin $br 2>&1 | Out-Null
  exit 3
}
$log += "installer: $($exe.Name) ($($exe.Length) bytes)"
Unblock-File $exe.FullName -ErrorAction SilentlyContinue
$sig = (Get-AuthenticodeSignature $exe.FullName).Status
...
if ($svc -and $kitVersion -and $installedVersion -and ($installedVersion -ne $kitVersion) -and $quietUninstall) {
  $log += "installed version differs from kit version -- uninstalling first"
  try {
    $cmd = $quietUninstall.Trim()
    if ($cmd.StartsWith('"')) {
      $endQuote = $cmd.IndexOf('"', 1)
      $file = $cmd.Substring(1, $endQuote - 1)
    ...
    $u = $(if ($args) { Start-Process -FilePath $file -ArgumentList $args -PassThru -Wait }
           else       { Start-Process -FilePath $file -PassThru -Wait })
    $log += "uninstaller exit=$($u.ExitCode)"
    Start-Sleep -Seconds 20
  } catch {
    $log += "uninstall attempt failed: $($_.Exception.Message)"
  }
} elseif ($svc) {
  $log += "same version (or no reliable version/uninstall string) -- upgrading in place, no uninstall"
}
...
Set-Content "$repo\soak\INSTALL-RESULT.md" -Value ($log -join "`n") -Encoding utf8
git -C $repo add soak/INSTALL-RESULT.md
git -C $repo commit --quiet -m "test: autorun-1 install result exit=$installerExit healthy=$healthy $stamp soak8-e1acfe6"
git -C $repo push --quiet origin $br 2>&1 | Out-Null
if (-not $healthy) { Reset-AutorunMarker }
