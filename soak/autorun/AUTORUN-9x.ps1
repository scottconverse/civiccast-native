# AUTORUN-9x (soak8-e1acfe6) -- read-only: is the caption-tap fix (PR #172, kit 91caebc) active?
...
$out   = @("# AUTORUN-9x soak #3 first-30-min relaunch diag (clean install of 91caebc)", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
...
$out += '```'
$out += "## every relaunch / stall / egress-state line since 20:10Z (timestamps matter)"
$out += '```'
$out += ($tail | Where-Object { $_ -match 'relaunch|stall|egress state ->|command .*start|worker exited|exit code|watchdog' -and $_ -match '^2026-09-05 (14|15):' } | Select-Object -Last 120 | ForEach-Object { $_.Substring(0, [Math]::Min(240, $_.Length)) })
$out += '```'
...
Set-Content "$repo\soak\DIAG-9x-$stamp.md" -Value ($out -join "`n") -Encoding utf8
git -C $repo add "soak/DIAG-9x-$stamp.md"
git -C $repo commit --quiet -m "test: autorun-9x caption-tap fix check $stamp soak8-e1acfe6"
