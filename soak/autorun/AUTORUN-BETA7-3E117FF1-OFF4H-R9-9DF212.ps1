...
$package = Join-Path $PSScriptRoot '..\beta7\3e117ff1fa9e06873ecec5b5b07f1360bc8b228d\off4h-r9-9df212'
$identityPath = Join-Path $package 'run-identity.json'
$expectedTask = 'CivicCast-Beta7-3e117ff1-OFF4H-R9-9DF212'
$expectedMission = 'BETA7-3E117FF1-OFF4H-R9-9DF212'
$expectedSource = '3e117ff1fa9e06873ecec5b5b07f1360bc8b228d'
$jobRelative = 'soak/beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d/physical-soak-off4h-r9-9df212/job.json'
$terminalJobName = 'physical-soak-off4h-r9-9df212-job.json'
...
function Get-VerifiedStartReceipt($Identity) {
    ...
    if ([string]$receipt.schema -cne 'civiccast-beta7-start-verified-v1' -or
        ...
        throw 'R9 start marker does not bind the exact remotely verified candidate receipt.'
    }
    return $receipt
}
...
if ($DryRun) {
    ...
    if (@($parseErrors).Count) { throw "R9 activation order has parse errors: $($parseErrors.Message -join '; ')" }
    ...
    if ($gitCommands.Count) { throw 'R9 launcher must not race the physical job for its Git checkout.' }
    if ($selfText -notmatch '(?s)Start-Beta7PhysicalOFF4HJob.*?AddMinutes\(5\).*?Get-VerifiedStartReceipt.*?terminalJobPath.*?did not publish and remotely verify its start receipt') {
        throw 'R9 launcher does not wait fail-closed for the physical job start-verification marker.'
    }
    ...
    if ([string]$id.task_name -cne $expectedTask -or
        ...
        throw 'R9 activation constants do not match the finalized package identity.'
    }
...
if ($env:COMPUTERNAME -ine 'DESKTOP-VBMA6O5') { throw 'This beta.7 R9 mission targets DESKTOP-VBMA6O5 only.' }
...
do {
    ...
    if (Test-Path -LiteralPath $terminalJobPath -PathType Leaf) {
        ...
        throw "R9 physical task terminated before its remote start receipt: state=$($terminal.job_state), error=$($terminal.error)"
    }
    Start-Sleep -Seconds 2
} while ([datetime]::UtcNow -lt $deadline)
if (-not $verifiedStart) { throw 'R9 physical task did not publish and remotely verify its start receipt within five minutes.' }
