# AUTORUN-9o (soak8-e1acfe6) -- read-only: is the caption-tap fix (PR #172, kit 91caebc) active?
...
$out   = @("# AUTORUN-9o caption-tap fix check (kit 91caebc)", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
...
$tail = @(Get-Content $log -Tail 6000 -ErrorAction SilentlyContinue)
...
$out += '```'
foreach ($k in 'caption_tap','stall','relaunch','critical') {
...
Set-Content "$repo\soak\DIAG-9o-$stamp.md" -Value ($out -join "`n") -Encoding utf8
git -C $repo add "soak/DIAG-9o-$stamp.md"
git -C $repo commit --quiet -m "test: autorun-9o caption-tap fix check $stamp soak8-e1acfe6"
