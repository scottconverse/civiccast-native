# AUTORUN-9r (soak8-e1acfe6) -- CLEAN REINSTALL of the 91caebc kit on this tester box.
...
if (-not $exe) {
  ...
  Save-Report -Lines $log -Name 'REINSTALL-RESULT-9r.md' -Message "test: autorun-9 blocked (no kit) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name 'REINSTALL-RESULT-9r.md' -Message "test: autorun-9 dry-run $stamp soak8-e1acfe6"
...
Save-Report -Lines $log -Name 'REINSTALL-RESULT-9r.md' -Message "test: autorun-9r clean reinstall exit=$installerExit healthy=$healthy $stamp soak8-e1acfe6"
