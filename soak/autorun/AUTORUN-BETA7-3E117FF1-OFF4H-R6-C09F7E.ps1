...
$package = Join-Path $PSScriptRoot '..\beta7\3e117ff1fa9e06873ecec5b5b07f1360bc8b228d\off4h-r6-c09f7e'
$identityPath = Join-Path $package 'run-identity.json'
$expectedTask = 'CivicCast-Beta7-3e117ff1-OFF4H-R6-C09F7E'
$expectedMission = 'BETA7-3E117FF1-OFF4H-R6-C09F7E'
$expectedSource = '3e117ff1fa9e06873ecec5b5b07f1360bc8b228d'
$jobRelative = 'soak/beta7-3e117ff1fa9e06873ecec5b5b07f1360bc8b228d/physical-soak-off4h-r6-c09f7e/job.json'
$terminalJobName = 'physical-soak-off4h-r6-c09f7e-job.json'
...
function Get-VerifiedStartReceipt($Identity) {
    ...
    if ([string]$receipt.schema -cne 'civiccast-beta7-start-verified-v1' -or
        ...
        throw 'R6 start marker does not bind the exact remotely verified candidate receipt.'
    }
    return $receipt
}
...
if ($DryRun) {
    ...
    if (@($parseErrors).Count) { throw "R6 activation order has parse errors: $($parseErrors.Message -join '; ')" }
    ...
    if ($gitCommands.Count) { throw 'R6 launcher must not race the physical job for its Git checkout.' }
    if ($selfText -notmatch '(?s)Start-Beta7PhysicalOFF4HJob.*?AddMinutes\(5\).*?Get-VerifiedStartReceipt.*?terminalJobPath.*?did not publish and remotely verify its start receipt') {
        throw 'R6 launcher does not wait fail-closed for the physical job start-verification marker.'
    }
    ...
    if ([string]$id.task_name -cne $expectedTask -or
        ...
        throw 'R6 activation constants do not match the finalized package identity.'
    }
...
do {
    ...
    if (Test-Path -LiteralPath $terminalJobPath -PathType Leaf) {
        ...
        throw "R6 physical task terminated before its remote start receipt: state=$($terminal.job_state), error=$($terminal.error)"
    }
    Start-Sleep -Seconds 2
} while ([datetime]::UtcNow -lt $deadline)
if (-not $verifiedStart) { throw 'R6 physical task did not publish and remotely verify its start receipt within five minutes.' }
