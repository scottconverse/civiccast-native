# AUTORUN-9zi (soak8-e1acfe6) -- first-admin + three channels + program cycling + start playout.
...
$log   = @("# AUTORUN-9zi station setup + three-channel soak (2h candidate e5020746)", "- mission: soak8-e1acfe6", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- kit: $kit", "- DryRun: $($DryRun.IsPresent)", "")
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-9zi blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  if ($DryRun) {
    ...
    Save-Report -Lines $log -Name "AUTORUN-9zi-DRYRUN-$stamp.md" -Message "test: autorun-9zi dry-run $stamp soak8-e1acfe6"
    exit 0
  }
  try {
...
Save-Report -Lines $log -Name "AUTORUN-9zi-RESULT-$stamp.md" -Message "test: autorun-9zi first-admin token_present=$([bool]$token) $stamp soak8-e1acfe6"
