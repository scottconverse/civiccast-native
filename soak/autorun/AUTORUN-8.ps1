# AUTORUN-8 (soak8-e1acfe6) -- first-admin + three channels + program cycling + start playout.
...
$log   = @("# AUTORUN-8 station setup + three-channel soak (2h candidate e5020746)", "- mission: soak8-e1acfe6", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- kit: $kit", "- DryRun: $($DryRun.IsPresent)", "")
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-8 blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  if ($DryRun) {
    ...
    Save-Report -Lines $log -Name "AUTORUN-8-DRYRUN-$stamp.md" -Message "test: autorun-8 dry-run $stamp soak8-e1acfe6"
    exit 0
  }
  try {
...
if (-not $token) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-8 blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-8 blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE.md' -Message "test: autorun-8 channel schedule $stamp soak8-e1acfe6"
...
Save-Report -Lines $startDoc -Name 'SOAK-START.md' -Message "test: autorun-8 soak start $stamp soak8-e1acfe6"
Save-Report -Lines $log -Name "AUTORUN-4-RESULT-$stamp.md" -Message "test: autorun-8 result $stamp soak8-e1acfe6"
exit 0
