# AUTORUN-10 (soak8-e1acfe6) -- first-admin + three channels + program cycling + start playout.
...
$log   = @("# AUTORUN-10 station setup + three-channel soak (2h candidate e5020746)", "- mission: soak8-e1acfe6", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- kit: $kit", "- DryRun: $($DryRun.IsPresent)", "")
...
if (-not $health -or $health.status -ne 'healthy') {
  $log += "STALLED: station is not healthy; AUTORUN-9 must succeed first. last health: $($health | ConvertTo-Json -Compress -Depth 4)"
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-10 blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  if ($DryRun) {
    ...
    Save-Report -Lines $log -Name "AUTORUN-10-DRYRUN-$stamp.md" -Message "test: autorun-10 dry-run $stamp soak8-e1acfe6"
    exit 0
  }
  try {
...
if (-not $token) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-10 blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-10 blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE.md' -Message "test: autorun-10 channel schedule $stamp soak8-e1acfe6"
...
Save-Report -Lines $startDoc -Name 'SOAK-START.md' -Message "test: autorun-10 soak start $stamp soak8-e1acfe6"
Save-Report -Lines $log -Name "AUTORUN-4-RESULT-$stamp.md" -Message "test: autorun-10 result $stamp soak8-e1acfe6"
exit 0
