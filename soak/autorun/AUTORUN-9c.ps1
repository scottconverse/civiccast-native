# AUTORUN-9c (soak8-e1acfe6) -- clear the persisted first-admin marker and restart the station.
...
$log   = @("# AUTORUN-9c station-state reset", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- DryRun: $DryRun", "")
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name 'STATE-RESET-RESULT.md' -Message "test: autorun-9c dry-run $stamp soak8-e1acfe6"
...
Save-Report -Lines $log -Name 'STATE-RESET-RESULT.md' -Message "test: autorun-9c state reset healthy=$healthy files=$($found.Count) $stamp soak8-e1acfe6"
