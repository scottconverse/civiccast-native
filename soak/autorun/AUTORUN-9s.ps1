# AUTORUN-9s (soak8-e1acfe6) -- clear the persisted first-admin marker and restart the station.
...
$log   = @("# AUTORUN-9s station-state reset", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- DryRun: $DryRun", "")
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name 'STATE-RESET-RESULT-9s.md' -Message "test: autorun-9s dry-run $stamp soak8-e1acfe6"
...
Save-Report -Lines $log -Name 'STATE-RESET-RESULT-9s.md' -Message "test: autorun-9s state reset healthy=$healthy files=$($found.Count) $stamp soak8-e1acfe6"
