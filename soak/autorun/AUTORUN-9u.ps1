# AUTORUN-9u (soak8-e1acfe6) -- three-channel soak with beta.5-shaped API bodies.
...
$log   = @("# AUTORUN-9e station setup + three-channel soak (2h candidate e5020746, beta.5 API bodies)", "- mission: soak8-e1acfe6", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- kit: $kit", "- DryRun: $($DryRun.IsPresent)", "")
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9u.md' -Message "test: autorun-9u blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9u.md' -Message "test: autorun-9u blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9u.md' -Message "test: autorun-9u blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
$log += '```json'
$log += (([ordered]@{ asset_id = '<asset_id>'; channel_id = '<channel_id>'; mode = 'premiere'; scheduled_at = '<iso8601 utc>'; duration_seconds = '<int, from ffprobe>'; notes = 'Soak8 AUTORUN-9u soak #3 fresh install of kit 91caebc' }) | ConvertTo-Json -Depth 4)
$log += '```'
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name "AUTORUN-9u-DRYRUN-$stamp.md" -Message "test: autorun-9u dry-run $stamp soak8-e1acfe6"
  exit 0
}

# --------------------------------- 7. upload -> package -> readiness -> approve
$t6NoteLog = Join-Path $env:TEMP "autorun-9e-asset-notes-$stamp.txt"
...
foreach ($clipPath in $stagedClips) {
  ...
  $aId = 'soak8-9u-' + (Get-Date -Format 'yyMMddHHmmss') + '-' + $aSuffix
  $aTitle = 'Soak8 9e Asset ' + [System.IO.Path]::GetFileNameWithoutExtension($clipPath) + ' ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')

  $aUp = Invoke-AssetUpload -BaseUrl $base -Token $token -AssetId $aId -Title $aTitle -FilePath $clipPath -TimeoutSec 900
  if ($aUp.status -ne 201) {
    $log += "asset upload FAILED clip=$clipPath status=$($aUp.status) body=$($aUp.body_raw)"
    continue
  }
  $log += "asset uploaded: $clipPath -> $aId"

  $aPkg = Invoke-CivicCastApi -Method 'Post' -Url "$base/api/staff/assets/$aId/package" -BearerToken $token -TimeoutSec 900
  if ($aPkg.status -ne 200) {
    $log += "asset package FAILED asset_id=$aId status=$($aPkg.status) body=$($aPkg.body_raw)"
    continue
  }
  ...
  $aState = $null
  do {
    $aGet = Invoke-CivicCastApi -Method 'Get' -Url "$base/api/staff/assets/$aId" -BearerToken $token
  ...
  if (-not $assetReady) {
    $log += "asset never became ready asset_id=$aId last_state=$aState"
    continue
  }

  $aAppr = Invoke-CivicCastApi -Method 'Post' -Url "$base/api/staff/publish/assets/$aId/approve" -BodyObj (@{ operator_id = 'soakadmin'; operator_display_name = 'Soak Operator' }) -BearerToken $token -TimeoutSec 900
  if ($aAppr.status -ne 200) {
    $log += "asset approve FAILED asset_id=$aId status=$($aAppr.status) body=$($aAppr.body_raw)"
    continue
  }
  $aDur = Get-Mp4DurationSeconds -FfprobeExe $ffprobeExe -FilePath $clipPath -DefaultSeconds 30
  $assets += [ordered]@{ id = $aId; duration_seconds = $aDur }
  $log += "asset ready: $aId (duration_seconds=$aDur, state=$aState)"
}

if ($assets.Count -eq 0) {
  $log += "STALLED: no assets reached the ready+approved state; scheduling/config/start skipped."
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9u.md' -Message "test: autorun-9u blocked (no assets ready) $stamp soak8-e1acfe6"
  exit 4
}
...
$sched = @("# soak8-e1acfe6 channel schedule (2h candidate e5020746, AUTORUN-9e)", "- host: $env:COMPUTERNAME", "- soak scheduling phase start (UTC): $($scheduleStart.ToString('o'))",
...
foreach ($c in $channelSpecs) {
  ...
  while (($cursor -lt $scheduleEnd) -and ($scheduled -lt $scheduleCapPerChannel)) {
    ...
    $itemBody = [ordered]@{
      ...
      notes            = 'Soak8 AUTORUN-9u soak #3 fresh install of kit 91caebc'
    }
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE-9u.md' -Message "test: autorun-9u channel schedule $stamp soak8-e1acfe6"
...
if ($scheduleTooLong) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9u.md' -Message "test: autorun-9u blocked (schedule too long) $stamp soak8-e1acfe6"
  exit 5
}
...
$onAirDeadline = (Get-Date).AddMinutes(3)
$anyOnAir = $false
do {
  foreach ($c in $channelSpecs) {
    try {
...
# ------------------------- 10b. archive the INVALID soak #2 probes (old code) + reset counters
if ($anyOnAir) {
  $arch = "$repo\soak\archive-91caebc-soak2-oldcode"
  ...
  $log += "soak #2 (old code) probes archived to soak/archive-91caebc-soak2-oldcode; counters reset"
}
...
if ($anyOnAir) {
  ...
  $startDoc = @("# soak8-e1acfe6 SOAK START (candidate e5020746, 2h, AUTORUN-9e)", "", "- host: $env:COMPUTERNAME",
  ...
  Save-Report -Lines $startDoc -Name 'SOAK-START-9u.md' -Message "test: autorun-9u soak start $stamp soak8-e1acfe6"
} else {
...
Save-Report -Lines $log -Name "AUTORUN-9u-RESULT-$stamp.md" -Message "test: autorun-9u result $stamp soak8-e1acfe6"
exit 0
