# AUTORUN-9 (soak8-e1acfe6) -- CLEAN REINSTALL of the e5020746 kit on this tester box.
# Why: the station carries an admin from the 2026-09-03 mission whose credentials were
...
if (-not $exe) {
  ...
  Save-Report -Lines $log -Name 'REINSTALL-RESULT.md' -Message "test: autorun-9 blocked (no kit) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name 'REINSTALL-RESULT.md' -Message "test: autorun-9 dry-run $stamp soak8-e1acfe6"
...
Save-Report -Lines $log -Name 'REINSTALL-RESULT.md' -Message "test: autorun-9 clean reinstall exit=$installerExit healthy=$healthy $stamp soak8-e1acfe6"
