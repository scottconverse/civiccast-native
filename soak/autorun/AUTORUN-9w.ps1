# AUTORUN-9w (soak8-e1acfe6) -- after the CLEAN reinstall of 91caebc (9u saw no ON_AIR within 3 min) the three channels did not come back
...
$log   = @("# AUTORUN-9w restart channels after the CLEAN reinstall of 91caebc (9u saw no ON_AIR within 3 min), then restart the soak", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
$channels = 'public','education','government'

function Publish-Log([string]$msg) {
  New-Item -Force -ItemType Directory "$repo\soak" | Out-Null
  Set-Content "$repo\soak\RESTART-RESULT-9w.md" -Value ($log -join "`n") -Encoding utf8
  git -C $repo add -A soak
  git -C $repo commit --quiet -m "test: autorun-9w $msg $stamp soak8-e1acfe6"
...
$log += "old probes archived to soak/archive-91caebc-soak2-oldcode; counters reset; soak #3 started $startIso on kit $sha"
Publish-Log "soak #3 STARTED on kit $sha after channel restart"
exit 0
