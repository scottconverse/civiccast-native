# AUTORUN-9zd (soak8-e1acfe6) -- three-channel soak with beta.5-shaped API bodies.
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9zd.md' -Message "test: autorun-9zd blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9zd.md' -Message "test: autorun-9zd blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9zd.md' -Message "test: autorun-9zd blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
$log += '```json'
$log += (([ordered]@{ asset_id = '<asset_id>'; channel_id = '<channel_id>'; mode = 'premiere'; scheduled_at = '<iso8601 utc>'; duration_seconds = '<int, from ffprobe>'; notes = 'Soak8 AUTORUN-9zd soak #5 fresh install of kit 609273d' }) | ConvertTo-Json -Depth 4)
$log += '```'
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name "AUTORUN-9zd-DRYRUN-$stamp.md" -Message "test: autorun-9zd dry-run $stamp soak8-e1acfe6"
  exit 0
}
...
if ($assets.Count -eq 0) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9zd.md' -Message "test: autorun-9zd blocked (no assets ready) $stamp soak8-e1acfe6"
  exit 4
}
...
foreach ($c in $channelSpecs) {
  ...
  while (($cursor -lt $scheduleEnd) -and ($scheduled -lt $scheduleCapPerChannel)) {
    ...
    $itemBody = [ordered]@{
      ...
      notes            = 'Soak8 AUTORUN-9zd soak #5 fresh install of kit 609273d'
    }
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE-9zd.md' -Message "test: autorun-9zd channel schedule $stamp soak8-e1acfe6"
...
if ($scheduleTooLong) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9zd.md' -Message "test: autorun-9zd blocked (schedule too long) $stamp soak8-e1acfe6"
  exit 5
}
...
if ($anyOnAir) {
  ...
  $log += "soak #2 (old code) probes archived to soak/archive-609273d-prev-soak; counters reset"
}
...
if ($anyOnAir) {
  ...
  Save-Report -Lines $startDoc -Name 'SOAK-START-9zd.md' -Message "test: autorun-9zd soak start $stamp soak8-e1acfe6"
} else {
...
Save-Report -Lines $log -Name "AUTORUN-9zd-RESULT-$stamp.md" -Message "test: autorun-9zd result $stamp soak8-e1acfe6"
exit 0
