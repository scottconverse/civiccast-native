...
$directiveRelative = 'soak/DIRECTIVE-BETA7-FINALIZE-R5-INVALIDATION-E27A91.md'
$scriptRelative = 'soak/autorun/AUTORUN-00-BETA7-FINALIZE-R5-INVALIDATION-E27A91.ps1'
...
function Get-R5LocalVerification {
    ...
    if (-not [string]$runIdentity.run_id -or
        [string]$runIdentity.mission_nonce -cne $missionNonce -or
    ...
    foreach ($entry in $plan) {
        ...
        if ([string]$entry.notes -cne "Owned $([string]$runIdentity.run_id)") { throw 'R5 published plan contains notes outside the exact run identity.' }
    }
    [pscustomobject][ordered]@{
...
function Publish-R5Invalidation($Verification, $DirectiveBinding) {
    ...
    $receipt = [pscustomobject][ordered]@{
        ...
        recovery_order = 'BETA7-FINALIZE-R5-INVALIDATION-E27A91'
