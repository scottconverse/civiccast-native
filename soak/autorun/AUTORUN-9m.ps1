# AUTORUN-9m (soak8-e1acfe6) -- three-channel soak with beta.5-shaped API bodies.
...
$log   = @("# AUTORUN-9m soak #2: reschedule the approved soak assets on kit 91caebc, start, restart the soak clock", "- mission: soak8-e1acfe6", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- kit: $kit", "- DryRun: $($DryRun.IsPresent)", "")
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9m.md' -Message "test: autorun-9m blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9m.md' -Message "test: autorun-9m blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9m.md' -Message "test: autorun-9m blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
$log += '```json'
$log += (([ordered]@{ asset_id = '<asset_id>'; channel_id = '<channel_id>'; mode = 'premiere'; scheduled_at = '<iso8601 utc>'; duration_seconds = '<int, from ffprobe>'; notes = 'Soak8 AUTORUN-9m soak #2 on kit 91caebc' }) | ConvertTo-Json -Depth 4)
$log += '```'
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name "AUTORUN-9m-DRYRUN-$stamp.md" -Message "test: autorun-9m dry-run $stamp soak8-e1acfe6"
  exit 0
}

# --------------------------------- 7. reuse the assets AUTORUN-9e uploaded + approved
# They live in Postgres and survived the upgrade. Pick every soak8-9e-* asset in a
...
foreach ($it in $items) {
  ...
  if ($id -notlike 'soak8-9e-*') { continue }
...
if ($assets.Count -eq 0) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9m.md' -Message "test: autorun-9m blocked (no assets ready) $stamp soak8-e1acfe6"
  exit 4
}
...
$sched = @("# soak8-e1acfe6 channel schedule (soak #2, kit 91caebc, AUTORUN-9m)", "- host: $env:COMPUTERNAME", "- soak scheduling phase start (UTC): $($scheduleStart.ToString('o'))",
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE-9m.md' -Message "test: autorun-9m channel schedule $stamp soak8-e1acfe6"
...
if ($scheduleTooLong) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED-9m.md' -Message "test: autorun-9m blocked (schedule too long) $stamp soak8-e1acfe6"
  exit 5
}
...
if ($anyOnAir) {
  ...
  $log += "soak #1 history archived to soak/archive-e502074-soak1; relaunch/pid/rollup counters reset"
}
...
if ($anyOnAir) {
  ...
  $startDoc = @("# soak8-e1acfe6 SOAK START (soak #2, kit 91caebc, 2h, AUTORUN-9m)", "", "- host: $env:COMPUTERNAME",
  ...
  Save-Report -Lines $startDoc -Name 'SOAK-START-9m.md' -Message "test: autorun-9m soak start $stamp soak8-e1acfe6"
} else {
...
Save-Report -Lines $log -Name "AUTORUN-9m-RESULT-$stamp.md" -Message "test: autorun-9m result $stamp soak8-e1acfe6"
exit 0
