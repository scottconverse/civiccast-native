...
if($SelfTest){
    ...
    if(-not $roundTrip.file.sha256 -or $roundTrip.file.tail.Length -gt 12000){throw 'Bounded evidence serialization failed.'}
...
$report=[ordered]@{
    order='FIXED-BETA-R3-FAILURE-DIAGNOSTIC-R1'
...
Write-Output 'FIXED_BETA_R3_FAILURE_DIAGNOSTIC_JSON_BEGIN'
Write-Output $json
Write-Output 'FIXED_BETA_R3_FAILURE_DIAGNOSTIC_JSON_END'
