# AUTORUN-9v (soak8-e1acfe6) -- read-only: which civiccast code is actually installed after the
...
$out   = @("# AUTORUN-9v installed-code check after the CLEAN reinstall of 91caebc (AUTORUN-9r)", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
...
if ($pack) {
  ...
  try {
    ...
    foreach ($e in $hits) {
      $tmp = Join-Path $env:TEMP ("9v-" + [IO.Path]::GetFileName($e.FullName))
...
Set-Content "$repo\soak\DIAG-9v-$stamp.md" -Value ($out -join "`n") -Encoding utf8
git -C $repo add "soak/DIAG-9v-$stamp.md"
git -C $repo commit --quiet -m "test: autorun-9v installed-code check $stamp soak8-e1acfe6"
