# AUTORUN-9e (soak8-e1acfe6) -- three-channel soak with beta.5-shaped API bodies.
...
if (-not $health -or $health.status -ne 'healthy') {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-9e blocked (station not healthy) $stamp soak8-e1acfe6"
  exit 1
}
...
if (-not $token) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-9e blocked (no token) $stamp soak8-e1acfe6"
  exit 2
}
...
if ($samples.Count -lt 1) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-9e blocked (no samples) $stamp soak8-e1acfe6"
  exit 3
}
...
$log += '```json'
$log += (([ordered]@{ asset_id = '<asset_id>'; channel_id = '<channel_id>'; mode = 'premiere'; scheduled_at = '<iso8601 utc>'; duration_seconds = '<int, from ffprobe>'; notes = 'Soak8 AUTORUN-9e three-channel soak' }) | ConvertTo-Json -Depth 4)
$log += '```'
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name "AUTORUN-9e-DRYRUN-$stamp.md" -Message "test: autorun-9e dry-run $stamp soak8-e1acfe6"
  exit 0
}
...
foreach ($clipPath in $stagedClips) {
  ...
  $aId = 'soak8-9e-' + (Get-Date -Format 'yyMMddHHmmss') + '-' + $aSuffix
...
if ($assets.Count -eq 0) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-9e blocked (no assets ready) $stamp soak8-e1acfe6"
  exit 4
}
...
Save-Report -Lines $sched -Name 'CHANNEL-SCHEDULE.md' -Message "test: autorun-9e channel schedule $stamp soak8-e1acfe6"
...
if ($scheduleTooLong) {
  ...
  Save-Report -Lines $log -Name 'SETUP-BLOCKED.md' -Message "test: autorun-9e blocked (schedule too long) $stamp soak8-e1acfe6"
  exit 5
}
...
if ($anyOnAir) {
  ...
  Save-Report -Lines $startDoc -Name 'SOAK-START.md' -Message "test: autorun-9e soak start $stamp soak8-e1acfe6"
} else {
...
Save-Report -Lines $log -Name "AUTORUN-9e-RESULT-$stamp.md" -Message "test: autorun-9e result $stamp soak8-e1acfe6"
exit 0
