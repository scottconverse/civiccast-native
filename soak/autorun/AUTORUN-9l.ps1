# AUTORUN-9l (soak8-e1acfe6) -- after the 91caebc upgrade the three channels did not come back
...
$log   = @("# AUTORUN-9l restart channels after the 91caebc upgrade, then restart the soak", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
$channels = 'public','education','government'

function Publish-Log([string]$msg) {
  New-Item -Force -ItemType Directory "$repo\soak" | Out-Null
  Set-Content "$repo\soak\RESTART-RESULT-9l.md" -Value ($log -join "`n") -Encoding utf8
  git -C $repo add -A soak
  git -C $repo commit --quiet -m "test: autorun-9l $msg $stamp soak8-e1acfe6"
...
Publish-Log "second soak STARTED on kit $sha after channel restart"
exit 0
