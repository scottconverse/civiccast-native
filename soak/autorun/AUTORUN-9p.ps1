# AUTORUN-9p (soak8-e1acfe6) -- read-only: which civiccast code is actually installed after the
...
$out   = @("# AUTORUN-9p installed-code check after the 91caebc upgrade", "- host: $env:COMPUTERNAME", "- utc: $stamp", "")
...
if (Test-Path $tw) { $c = Get-Content $tw -Raw; $out += "  has 'CaptionBackoffPolicy': $($c -match 'CaptionBackoffPolicy')  has 'state=\"paused\"': $($c -match 'paused')  has '_LOG.critical': $($c -match '_LOG\.critical')" }
$out += '```'
...
if ($pack) {
  ...
  try {
    ...
    foreach ($e in $hits) {
      $tmp = Join-Path $env:TEMP ("9p-" + [IO.Path]::GetFileName($e.FullName))
...
Set-Content "$repo\soak\DIAG-9p-$stamp.md" -Value ($out -join "`n") -Encoding utf8
git -C $repo add "soak/DIAG-9p-$stamp.md"
git -C $repo commit --quiet -m "test: autorun-9p installed-code check $stamp soak8-e1acfe6"
