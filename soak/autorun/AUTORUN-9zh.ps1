# AUTORUN-9zh (soak8-e1acfe6) -- clear the persisted first-admin marker and restart the station.
...
$log   = @("# AUTORUN-9zh station-state reset", "- host: $env:COMPUTERNAME", "- utc: $stamp", "- DryRun: $DryRun", "")
...
if ($DryRun) {
  ...
  Save-Report -Lines $log -Name 'STATE-RESET-RESULT-9zh.md' -Message "test: autorun-9zh dry-run $stamp soak8-e1acfe6"
...
Save-Report -Lines $log -Name 'STATE-RESET-RESULT-9zh.md' -Message "test: autorun-9zh state reset healthy=$healthy files=$($found.Count) $stamp soak8-e1acfe6"
