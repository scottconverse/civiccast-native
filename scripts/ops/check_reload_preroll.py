...
_READY = re.compile(
    r"preroll verified \(reload_id=(\d+)\) held_streams=(\d+) "
    r"timing=(finite|clock) mode=(immediate|deferred)"
)
_HELD = re.compile(r"\(0 stream\(s\) still to preroll\) \(reload_id=(\d+)\)")
_REBASED = re.compile(r"finite switch rebased .* reload_id=(\d+)")
...
def check_log(text: str, expected_elements: int | None = None) -> tuple[int, list[str]]:
    ...
    Held finite legs additionally require the final all-stream hold line. Live
    clock-timed legs use the engine's all-stream first-buffer proof; inventing a
    hold for those streams would change their clock behavior.
    """
    held: set[int] = set()
    ready: dict[int, str] = {}
    rebased: set[int] = set()
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        if match := _READY.search(line):
            txn, streams, timing = int(match[1]), int(match[2]), match[3]
            if timing == "finite" and streams == 0:
                errors.append(f"line {number}: finite reload {txn} has zero held streams")
            elif timing == "finite" and txn not in held:
                errors.append(f"line {number}: reload {txn} has no completed preroll hold")
            elif timing == "clock" and streams != 0:
                errors.append(f"line {number}: clock-timed reload {txn} unexpectedly held streams")
            else:
                ready[txn] = timing
        if match := _REBASED.search(line):
            rebased.add(int(match[1]))
        ...
        if match := _COMMIT.search(line):
            ...
            if firing is None or firing not in ready:
                ...
                timing = ready.pop(firing)
                if timing == "finite" and firing not in rebased:
                    errors.append(
                        f"line {number}: finite reload {firing} has no matching rebase proof"
                    )
                held.discard(firing)
                rebased.discard(firing)
            firing = None
