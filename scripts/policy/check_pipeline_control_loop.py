...
def validate_control_state(fields: dict[str, str]) -> list[str]:
    ...
    if stop_condition == "none":
        if final_allowed != "false":
            violations.append(
                "`final_response_allowed` must be `false` when `stop_condition` is `none`"
            )
    ...
    if active_run == "true" and final_allowed == "true" and stop_condition == "none":
        violations.append(
            "active runs cannot allow a final response without a valid stop condition"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--report", action="append", default=[], help="Report file to scan for open caveats."
    )
    parser.add_argument(
        "--print-policy", action="store_true", help="Print stop-condition policy and exit."
    )
