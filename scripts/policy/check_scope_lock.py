...
try:
    ...
    from scripts.policy.scope_lock_utils import (
        find_term_owner,
        list_value,
        load_scope_lock,
        normalize_text,
...
def _title_without_estimate(value: str) -> str:
...
def evaluate_scope_lock(run_id: str, run_dir: Path = RUN_DIR, root: Path = REPO_ROOT) -> list[str]:
    ...
    try:
        ...
        return [f"scope-lock.yaml missing at {exc.args[0]}. Create it before product work starts."]
    ...
    if _title_without_estimate(section.title) != _title_without_estimate(rung_title):
        violations.append(
    ...
    for module in list_value(lock, "required_modules"):
        if normalize_text(module) not in normalize_text(f"{section.title}\n{section.body}"):
            violations.append(
...
def main() -> int:
    ...
    if not args.run:
        print(
            "check_scope_lock: no --run argument provided; skipping (no-op outside a pipeline run)."
        )
        return 0
