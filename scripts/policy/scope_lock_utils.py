...
try:
    ...
    from scripts.policy.policy_utils import find_repo_root, strip_yaml_comment


RUNG_HEADING = re.compile(
    r"^(?P<hashes>#{1,6})\s*(?:(?:rung\s*)?(?:v)?(?P<numeric_rung>\d+(?:\.\d+)*)|(?P<wp_rung>wp\d+))\s*(?:[-:\u2014\u2013]\s*(?P<title>.+?))?\s*$",
    re.IGNORECASE,
)
...
def parse_release_plan(path: Path) -> dict[str, ReleaseRung]:
    ...
    for index, line in enumerate(lines):
        ...
        if match:
            numeric_rung = match.group("numeric_rung")
            wp_rung = match.group("wp_rung")
            matches.append(
                (
                    index,
                    len(match.group("hashes")),
                    numeric_rung or wp_rung.upper(),
