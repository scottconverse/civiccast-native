...
def _path_text(path: str) -> str:
    return normalize_text(
        path.replace("/", " ").replace("\\", " ").replace("-", " ").replace("_", " ")
    )
...
def evaluate_rung_file_ownership(
    ...
    check_paths = (
        ...
        else [path for path in changed_paths(root) if (root / path).exists()]
    )
    ...
    if subject and (commit_message is not None or not check_paths):
...
def main() -> int:
    ...
    parser.add_argument(
        "--commit-message", help="Commit subject to classify before commit-msg hooks."
    )
    args = parser.parse_args()

    if not args.run:
        print(
            "check_rung_file_ownership: no --run argument provided; skipping (no-op outside a pipeline run)."
        )
        return 0
