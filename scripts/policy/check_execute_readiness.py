...
This check prevents a useful implementation slice from being treated as the
whole CivicCast rung. Backend tests, local lint, or a partial report are not
enough to advance when the manifest still requires product UX, docs, evidence,
persistence, browser QA, CI, or prior blocker fixes.
"""
...
def check_execute_readiness(run_id: str) -> list[str]:
    ...
    if READY_LINE not in text:
        if NOT_READY_LINE in text:
            ...
            violations.append(
                "implementation-report.md missing exact readiness line `**DoD readiness: READY**`."
            )
    ...
    if not match:
        ...
        if blocked:
            violations.append(
                f"DoD checklist still has {blocked} blocked item(s); execute is not complete."
            )
...
def main() -> int:
    ...
    if not args.run:
        print(
            "check_execute_readiness: no --run argument provided; "
            "skipping (no-op outside a pipeline run)."
        )
        return 0
