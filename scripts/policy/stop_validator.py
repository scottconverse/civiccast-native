...
def validate_state_file(state_path: Path) -> StopValidation:
    ...
    if fields.get("active_run", "").lower() != "true":
        return StopValidation(allowed=True, reason="inactive run", state_path=state_path)
...
def _validate_specific_stop(run_dir: Path, fields: dict[str, str]) -> str:
    ...
    if stop_condition == "scope_conflict":
        return _validate_scope_conflict(run_dir, current_stage, next_required_action, continuing_to)
...
def _validate_scope_conflict(
    ...
    if _scope_lock_receipt_passes(run_dir):
        action_text = " ".join([current_stage, next_required_action, continuing_to]).lower()
...
def _validate_failed_gate(run_dir: Path, current_stage: str) -> str:
    ...
    if events and not any(
        event.stage == current_stage and event.status in {"FAILED", "BLOCKED"} for event in events
    ):
        return (
...
def _require_text_signal(
    ...
    if any(needle in haystack for needle in needles):
        return ""
    return (
        f"{stop_condition} is unproven: next_required_action/continuing_to lack a matching signal"
    )
...
def _resume_stage(run_dir: Path) -> str:
    ...
    if not stages:
        return ""
    completed = {event.stage for event in _read_run_log(run_dir) if event.status == "COMPLETE"}
    for stage in stages:
