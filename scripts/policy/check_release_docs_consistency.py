...
def evaluate_release_docs_consistency(
    ...
    historical_evidence_paths = list_value(lock, "historical_evidence_paths")
    rung_markers = [f"v{current_rung}", current_rung]
    violations: list[str] = []

    for path in _doc_paths(root, canonical_source):
        relative_path = path.relative_to(root).as_posix()
        if any(fnmatch.fnmatch(relative_path, pattern) for pattern in historical_evidence_paths):
            continue
...
def main() -> int:
    ...
    if not args.run:
        print(
            "check_release_docs_consistency: no --run argument provided; skipping (no-op outside a pipeline run)."
        )
        return 0
