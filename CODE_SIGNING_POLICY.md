...
> **Native-chain note (chore/retire-wsl-lane):** the paragraph below describes the retired
> `release-artifacts.yml` pipeline. The current native chain (`native-beta-candidate-artifacts.yml`
> + `sign-native-installer.yml`) performs Authenticode signing only — it has no cosign/sigstore
> step today. Whether Sigstore provenance is reintroduced for the native chain is an open owner
> decision, not yet resolved; do not assume the native installer carries a `.sigstore.json` bundle
