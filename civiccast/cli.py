...
def _quote_env_file_value(value: str) -> str:
    return shlex.quote(value)


def _write_egress_env_file(path: Path, values: dict[str, str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "".join(f"{key}={_quote_env_file_value(value)}\n" for key, value in values.items()),
        encoding="utf-8",
    )
    if os.name != "nt":
        path.chmod(0o640)
...
def _write_egress_unit_file(path: Path, unit_text: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(unit_text, encoding="utf-8")


def _run_systemctl(args: list[str]) -> None:
    subprocess.run(["/bin/systemctl", *args], check=True, capture_output=True, text=True)  # noqa: S603
...
@egress_app.command("enable")
def egress_enable(
    channel_id: Annotated[str, typer.Option("--channel-id", help="Channel id to enable.")],
    civiccast_bin: Annotated[
        Path,
        typer.Option("--civiccast-bin", help="Absolute path to the CivicCast CLI binary."),
    ] = Path("/usr/bin/civiccast"),
    work_dir: Annotated[
        Path,
        typer.Option("--work-dir", help="Egress worker directory used by the system service."),
    ] = Path("/var/lib/civiccast/egress"),
    env_dir: Annotated[
        Path,
        typer.Option("--env-dir", help="Directory for per-channel egress environment files."),
    ] = Path("/etc/civiccast"),
    unit_path: Annotated[
        Path,
        typer.Option("--unit-path", help="Path where the systemd unit file is installed."),
    ] = Path("/etc/systemd/system/civiccast-egress@.service"),
    poll_seconds: Annotated[
        float,
        typer.Option("--poll-seconds", help="Seconds to wait between daemon polling passes."),
    ] = 2.0,
    secret_ref: Annotated[
        list[str] | None,
        typer.Option(
            "--secret-ref",
            help="Environment variable name to copy into the per-channel service env file.",
        ),
    ] = None,
    dry_run: Annotated[
        bool,
        typer.Option(
            "--dry-run", help="Print the plan without writing files or running systemctl."
        ),
    ] = False,
    json_output: Annotated[
        bool,
    ...
    """Install and enable the supervised systemd egress worker for one channel."""

    normalized_channel_id = channel_id.strip()
    if not normalized_channel_id:
        raise typer.BadParameter("--channel-id is required.")
    if not civiccast_bin.as_posix().startswith("/"):
        raise typer.BadParameter("--civiccast-bin must be an absolute Linux path.")
    if not work_dir.as_posix().startswith("/"):
        raise typer.BadParameter("--work-dir must be an absolute Linux path.")
    if not env_dir.is_absolute() and not env_dir.as_posix().startswith("/"):
        raise typer.BadParameter("--env-dir must be an absolute path.")
    if not unit_path.is_absolute() and not unit_path.as_posix().startswith("/"):
        raise typer.BadParameter("--unit-path must be an absolute path.")
    if poll_seconds < 0:
        raise typer.BadParameter("--poll-seconds must be zero or greater.")

    from civiccast.egress.service_unit import build_egress_systemd_unit

    unit = build_egress_systemd_unit(
        civiccast_bin=civiccast_bin.as_posix(),
        work_dir=work_dir.as_posix(),
        poll_seconds=poll_seconds,
    )
    service_instance = f"civiccast-egress@{normalized_channel_id}.service"
    env_path = env_dir / f"egress-{normalized_channel_id}.env"
    database_url = (
        os.environ.get("DATABASE_URL", "<installer-managed-database-url>")
        if dry_run
        else _resolve_egress_database_url()
    )
    env_values = {
        "DATABASE_URL": database_url,
        "CIVICCAST_EGRESS_WORK_DIR": work_dir.as_posix(),
    }
    missing_secret_refs: list[str] = []
    for ref in secret_ref or []:
        ref_name = ref.strip()
        if not ref_name:
            continue
        value = os.environ.get(ref_name)
        if value is None:
            missing_secret_refs.append(ref_name)
        elif not dry_run:
            env_values[ref_name] = value
    if missing_secret_refs and not dry_run:
        raise typer.BadParameter(
            "Missing secret environment variables: " + ", ".join(missing_secret_refs)
        )

    payload = {
        "channel_id": normalized_channel_id,
        "service": service_instance,
        "unit_path": _display_path(unit_path),
        "env_path": _display_path(env_path),
        "work_dir": _display_path(work_dir),
        "systemctl": [
            ["daemon-reload"],
            ["enable", "--now", service_instance],
            ["is-active", "--quiet", service_instance],
        ],
        "secret_refs": [ref for ref in (secret_ref or []) if ref.strip()],
    ...
    if dry_run:
        if json_output:
            ...
            typer.echo(f"Would enable {service_instance}.")
            typer.echo(f"Unit: {unit_path}")
            typer.echo(f"Environment: {env_path}")
        return

    _write_egress_unit_file(unit_path, unit.unit_text)
    _write_egress_env_file(env_path, env_values)
    for args in cast(list[list[str]], payload["systemctl"]):
        _run_systemctl(args)
    if json_output:
        typer.echo(json.dumps(payload, indent=2))
        return
    typer.echo(f"Enabled {service_instance}.")
    typer.echo(f"Unit: {unit_path}")
    typer.echo(f"Environment: {env_path}")


@egress_app.command("recovery-proof")
def egress_recovery_proof(
    channel_id: Annotated[
        str,
        typer.Option("--channel-id", help="Channel id measured during the recovery proof."),
    ],
    measured_recovery_seconds: Annotated[
        float,
        typer.Option(
            "--measured-seconds", help="Observed seconds until the channel returned on air."
        ),
    ],
    state_before: Annotated[
        str,
        typer.Option("--state-before", help="Egress state before killing the supervised worker."),
    ] = "ON_AIR",
    state_after: Annotated[
        str,
        typer.Option("--state-after", help="Egress state after the supervised worker recovered."),
    ] = "ON_AIR",
    sink_connected_after: Annotated[
        bool,
        typer.Option(
            "--sink-connected-after/--sink-not-connected-after",
            help="Whether the receiver/sink was connected after recovery.",
        ),
    ] = False,
    killed_pid: Annotated[
        int | None,
        typer.Option("--killed-pid", help="Process id killed during the proof, if recorded."),
    ] = None,
    replacement_pid: Annotated[
        int | None,
        typer.Option(
            "--replacement-pid", help="Replacement process id after recovery, if recorded."
        ),
    ] = None,
    evidence_path: Annotated[
        str | None,
        typer.Option("--evidence-path", help="Path to local tester evidence, if recorded."),
    ] = None,
    json_output: Annotated[
        bool,
        typer.Option("--json", help="Emit machine-readable JSON."),
    ...
    """Classify a real egress worker kill/restart measurement."""

    from civiccast.egress import build_recovery_observation, evaluate_recovery_observation

    observation = build_recovery_observation(
        channel_id=channel_id,
        measured_recovery_seconds=measured_recovery_seconds,
        ...
        sink_connected_after=sink_connected_after,
        evidence_path=evidence_path,
    )
    proof = evaluate_recovery_observation(observation)
    if json_output:
        typer.echo(proof.model_dump_json(indent=2))
        return
    typer.echo(f"Egress recovery proof: {proof.status}")
    typer.echo(f"Channel: {proof.channel_id}")
    typer.echo(
        "Measured recovery: "
        f"{proof.measured_recovery_seconds:g}s "
        f"(allowed {proof.allowed_recovery_window_seconds}s)"
    )
    typer.echo(f"State after recovery: {proof.state_after}")
    typer.echo(
        f"Receiver connected after recovery: {'yes' if proof.sink_connected_after else 'no'}"
    )
    if proof.blocker:
