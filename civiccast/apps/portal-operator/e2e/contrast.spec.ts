...
/**
 * S20 DC-4 — WCAG 2.1 AA contrast gate for the operator console.
 *
 * axe-core (`a11y.spec.ts`) already flags the `color-contrast` rule on
 * whatever it happens to scan, but that check is opportunistic: it only
 * ever complains, it never tells you *how many* elements it actually
 * looked at. A silently-empty scan (wrong selector, page not rendered
 * yet, JS error before paint) reads exactly like a clean pass — the
 * single most dangerous failure mode for an accessibility gate.
 *
 * This spec computes contrast directly from the rendered DOM/CSSOM
 * (foreground `color` vs the nearest non-transparent ancestor
 * `background-color`, walking up to the CSS canvas default of white)
 * against the WCAG 2.1 AA thresholds — 4.5:1 for normal text, 3:1 for
 * large text (>=24px, or >=18.66px at bold) — and refuses to report a
 * pass when the scanned element set is empty (the honesty guard named
 * in ROADMAP.status.yaml's S20 row).
 */
...
/** Runs inside the browser context — pure DOM + CSSOM, no network calls. */
async function scanContrast(page: Page, selector = 'body *'): Promise<ContrastFinding[]> {
  return page.evaluate((sel) => {
    ...
    function effectiveBackground(el: Element): [number, number, number] {
      ...
      return [255, 255, 255] // CSS canvas default background
    }
...
/**
 * The honesty guard: an empty scan is NEVER a pass. Throws instead of
 * returning `[]`, so a broken selector or an unrendered page fails loud
 * rather than reporting zero violations.
 */
  ...
  if (findings.length === 0) {
    throw new Error(
      `Contrast gate honesty guard: 0 elements survived the text/visibility/color filters for selector "${selector}". ` +
        'An empty scan is not a pass — it means the page did not render the text ' +
        'this gate expects (wrong selector, unrendered state, or a JS error before ' +
        'paint). Fix the page/selector rather than trusting a silent green.',
    )
  }
  return findings
}
...
test.describe('operator console contrast gate (WCAG 2.1 AA, S20 DC-4)', () => {
  test('first-setup screen meets 4.5:1 / 3:1 thresholds', async ({ page, scanContrast }) => {
    await page.goto('/')
  ...
  test('mobile first-setup screen meets thresholds', async ({ page, scanContrast }) => {
  ...
  test('asset library screen meets thresholds', async ({ page, scanContrast }) => {
    await page.goto('/')
    await page.getByRole('button', { name: 'Assets' }).click()
    await expect(page.locator('h1')).toBeVisible() // wait for the library to render before scanning
  ...
  test('gate catches an injected low-contrast element (falsification proof)', async ({
    ...
    // Append a synthetic element with a deliberately illegal ratio
    // (near-white text on white, ~1.1:1) — proves the gate has teeth
    // instead of trusting that "no violations found" means "checked and
    // clean." This does not touch any real app component.
    await page.evaluate(() => {
