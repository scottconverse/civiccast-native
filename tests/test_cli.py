...
def test_egress_enable_dry_run_json_redacts_secret_refs(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("DATABASE_URL", "sqlite:///civiccast.db")
    monkeypatch.setenv("EGRESS_SRT_PASSPHRASE", "station-passphrase")
    runner = CliRunner()
    ...
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["service"] == "civiccast-egress@council.service"
    assert payload["env_path"] == "/etc/civiccast/egress-council.env"
    assert payload["secret_refs"] == ["EGRESS_SRT_PASSPHRASE"]
    assert payload["secrets_redacted"] is True
    assert "station-passphrase" not in result.stdout


def test_egress_enable_writes_unit_env_and_runs_systemctl(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("DATABASE_URL", "sqlite:///civiccast.db")
    ...
    monkeypatch.setattr(cli_module, "_run_systemctl", lambda args: systemctl_calls.append(args))
    env_dir = tmp_path / "etc" / "civiccast"
    unit_path = tmp_path / "systemd" / "civiccast-egress@.service"
    runner = CliRunner()
    ...
    result = runner.invoke(
        ...
        [
            ...
            "council",
            "--civiccast-bin",
            "/opt/civiccast/venv/bin/civiccast",
            "--work-dir",
            "/var/lib/civiccast/egress",
            "--env-dir",
            str(env_dir).replace("\\", "/"),
            "--unit-path",
            str(unit_path).replace("\\", "/"),
            "--secret-ref",
            "EGRESS_SRT_PASSPHRASE",
            "--json",
        ],
    ...
    assert result.exit_code == 0, result.stdout
    payload = json.loads(result.stdout)
    env_path = env_dir / "egress-council.env"
    assert payload["service"] == "civiccast-egress@council.service"
    assert env_path.exists()
    assert "DATABASE_URL=sqlite:///civiccast.db" in env_path.read_text(encoding="utf-8")
    assert "EGRESS_SRT_PASSPHRASE=station-passphrase" in env_path.read_text(encoding="utf-8")
    assert "EnvironmentFile=/etc/civiccast/egress-%i.env" in unit_path.read_text(encoding="utf-8")
    assert systemctl_calls == [
        ["daemon-reload"],
        ["enable", "--now", "civiccast-egress@council.service"],
        ["is-active", "--quiet", "civiccast-egress@council.service"],
    ]
    assert "station-passphrase" not in result.stdout
...
def test_egress_recovery_proof_json() -> None:
    runner = CliRunner()
    ...
    assert payload["killed_pid"] == 100
    assert payload["replacement_pid"] == 200
    assert payload["checks"]["started_on_air"] is True
    assert payload["checks"]["returned_on_air"] is True
    assert payload["checks"]["within_recovery_window"] is True
    assert payload["checks"]["sink_connected_after"] is True
    assert payload["blocker"] is None


def test_egress_recovery_proof_reports_failures_in_plain_english() -> None:
    runner = CliRunner()
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--measured-seconds",
            "30",
            "--sink-not-connected-after",
        ],
    )

    assert result.exit_code == 0
    assert "Egress recovery proof: FAIL" in result.stdout
    assert "Blocker: EGRESS_RECOVERY_PROOF_FAILED_" in result.stdout
