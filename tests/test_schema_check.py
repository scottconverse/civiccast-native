...
def test_expected_head_matches_the_single_migration_head() -> None:
    ...
    # after 0074. 0076_agenda_item_confidence (product-hole fix: adds
    # agenda_items.confidence for the PDF-agenda-import heuristic) is the
    # current head, chained after 0075.
    assert expected_migration_head() == "0076_agenda_item_confidence"
...
def test_schema_check_reports_current_from_non_repo_working_directory(
    ...
    assert status == SchemaStatus(
        state="current",
        db_revision="0076_agenda_item_confidence",
        expected_head="0076_agenda_item_confidence",
    )
