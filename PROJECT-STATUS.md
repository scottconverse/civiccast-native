...
- **Authority is granted explicitly, per action.** Do not open pull requests, push branches, or
  write to the repository on your own initiative. A PR costs a full CI cycle (~40 minutes for
  `Unit tests`). Ask first, every time. Added 2026-09-10 after a session opened a PR for a
  two-paragraph documentation fix without being asked.
