...
## Not done
- MAIN WAS RED (11:15 PM): `tests/policy/test_windows_release_downloader.py::test_windows_install_doc_matches_current_release_posture`, `tests/test_audit_protocol_docs.py::test_active_public_docs_link_validated_current_candidate`, and `tests/policy/test_release_truth.py::test_real_manifest_is_internally_consistent`. Two causes: (a) README.md / INSTALL-WINDOWS.md / docs/index.html / docs/install-windows.html still named v1.0.0-beta.4 as the current download while release-truth says beta.5; (b) release-truth.yaml carried DUPLICATE v1.0.0-beta.5 entries (one `current` from the publisher, one leftover `staging`) so check_release_truth.py reported DRIFT. FIXED 12:20 AM in #165 (head cd50c3d4): surface flip committed as baf94d62 and origin/main merged in. Locally `tests/policy` is 1920 passed / 5 skipped and `check_release_truth.py` says PASS. Every other PR inherited this redness; re-judge them only after #165 merges.
- PR #213's `tests/policy/test_staff_mutation_role_policy.py::test_every_staff_mutation_route_has_a_role_marker` failure is NOT inherited -- that test passes on #165's tree. #213 adds a staff mutation route without a role marker and must fix it itself.
