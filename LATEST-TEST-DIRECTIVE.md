...
Current: `soak/DIRECTIVE-BETA7-FINALIZE-R5-INVALIDATION-E27A91C.md`

Autorun: `soak/autorun/AUTORUN-00-BETA7-FINALIZE-R5-INVALIDATION-E27A91C.ps1`
...
older per-run `IDENTITY.json`; the second exposed Windows PowerShell 5.1's
top-level JSON-array behavior. Both halted before writing. This corrected
once-only order explicitly enumerates every plan row, verifies the exact R5
task, processes, channel configs/states, and owned schedule rows, then
publishes and remotely verifies only `job.json` and `INVALIDATED.json`.
