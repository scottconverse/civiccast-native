...
Updated: 2026-09-05T08:30Z (rev 11 - AUTORUN-9c clears the service-profile station-state.json (first-admin 409 after a clean reinstall = leftover marker), AUTORUN-9d channels)
Platform prompt: PROMPT-WINDOWS-CODEX.md (the one paste for a Codex-desktop tester on a Windows box)
Autoruns queued: soak/autorun/AUTORUN-5.ps1 (fetch the e5020746 kit into a fresh kit-<sha> folder, verify, install OVER the existing station), soak/autorun/AUTORUN-9d.ps1 (after AUTORUN-9 clean reinstall; first-admin + three channels + start, 2h15m schedule), soak/autorun/AUTORUN-3.ps1 (TSDuck egress proof, engine-per-channel, relaunch tracking, worker CPU/RSS, 30-min rollups, T+2h verdict)
